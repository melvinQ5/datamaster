...
7天曝光sku占比=（在终审时间7天内的广告表现数据，且曝光量大于100的sku数） ÷（当周开发的sku数）
SKU曝光 >100个 算有效曝光，比如sku10000.01 在7天内曝光量累计达到100，sku10000.01就被有效曝光


注意： 美工编辑人员名单是业务部门给到的名单硬编码写入，后期可能遇到人员调整。不含名单外参与过编辑的人员，本数据也是为了评估目前在工作的人 
*/
...
-- 表1 美编处理sku明细
-- select t.dev_week`开发周` ,t.BoxSku, t.DevelopLastAuditTime `开发终审时间`, ad.Product_Artist `美工`, ad.Product_Editor `编辑`
-- 	-- 曝光量
-- 	, round(sum(case when 0 < ad_days and ad_days <= 7 then Exposure end)) as `7天曝光量` -- ad7_sku_Exposure 
-- 	, round(sum(case when 0 < ad_days and ad_days <= 14 then Exposure end)) as `14天曝光量` -- ad14_sku_Exposure
-- 	, round(sum(case when 0 < ad_days and ad_days <= 30 then Exposure end)) as `30天曝光量` -- ad30_sku_Exposure
-- 	, round(sum(case when 0 < ad_days and ad_days <= 60 then Exposure end)) as `60天曝光量` -- ad60_sku_Exposure
-- 	, round(sum(case when 0 < ad_days and ad_days <= 90 then Exposure end)) as `90天曝光量` -- ad90_sku_Exposure
-- 	, round(sum(case when 0 < ad_days and ad_days <= 120 then Exposure end)) as `120天曝光量` -- ad120_sku_Exposure
-- 	-- 点击量
-- 	, round(sum(case when 0 < ad_days and ad_days <= 7 then Clicks end)) as `7天点击量` -- ad7_sku_Clicks
-- 	, round(sum(case when 0 < ad_days and ad_days <= 14 then Clicks end)) as `14天点击量` -- ad14_sku_Clicks
-- 	, round(sum(case when 0 < ad_days and ad_days <= 30 then Clicks end)) as `30天点击量` -- ad30_sku_Clicks
-- 	, round(sum(case when 0 < ad_days and ad_days <= 60 then Clicks end)) as `60天点击量` -- ad60_sku_Clicks
-- 	, round(sum(case when 0 < ad_days and ad_days <= 90 then Clicks end)) as `90天点击量` -- ad90_sku_Clicks
-- 	, round(sum(case when 0 < ad_days and ad_days <= 120 then Clicks end)) as `120天点击量` -- ad120_sku_Clicks
-- 	-- 销量	
-- 	, round(sum(case when 0 < ad_days and ad_days <= 7 then TotalSale7DayUnit end)) as `7天销量` -- ad7_sku_TotalSale7DayUnit
-- 	, round(sum(case when 0 < ad_days and ad_days <= 14 then TotalSale7DayUnit end)) as `14天销量` -- ad14_sku_TotalSale7DayUnit
-- 	, round(sum(case when 0 < ad_days and ad_days <= 30 then TotalSale7DayUnit end)) as `30天销量` -- ad30_sku_TotalSale7DayUnit
-- 	, round(sum(case when 0 < ad_days and ad_days <= 60 then TotalSale7DayUnit end)) as `60天销量` -- ad60_sku_TotalSale7DayUnit
-- 	, round(sum(case when 0 < ad_days and ad_days <= 90 then TotalSale7DayUnit end)) as `90天销量` -- ad90_sku_TotalSale7DayUnit
-- 	, round(sum(case when 0 < ad_days and ad_days <= 120 then TotalSale7DayUnit end)) as `120天销量` -- ad120_sku_TotalSale7DayUnit
...
union all 
	select '日期\美工' `分析维度`, dev_week `开发周`, Product_Artist `美工`, '合计' `编辑`
		, dev_cnt `sku数`
		, round(ad7_sku_cnt/dev_cnt,4) as `7天曝光SKU占比`, round(ad14_sku_cnt/dev_cnt,4) as `14天曝光SKU占比`, round(ad30_sku_cnt/dev_cnt,4) as `30天曝光SKU占比`
		, round(ad60_sku_cnt/dev_cnt,4) as `60天曝光SKU占比`, round(ad90_sku_cnt/dev_cnt,4) as `90天曝光SKU占比`, round(ad120_sku_cnt/dev_cnt,4) as `120天曝光SKU占比`
		, round(ad7_sku_Clicks/ad7_sku_Exposure,4) as `7天点击率`, round(ad14_sku_Clicks/ad14_sku_Exposure,4) as `14天点击率`, round(ad30_sku_Clicks/ad30_sku_Exposure,4) as `30天点击率`
...
union all 
	select '日期\编辑' `分析维度`, dev_week `开发周`,  '合计' `美工`, Product_Editor `编辑`
		, dev_cnt `sku数`
		, round(ad7_sku_cnt/dev_cnt,4) as `7天曝光SKU占比`, round(ad14_sku_cnt/dev_cnt,4) as `14天曝光SKU占比`, round(ad30_sku_cnt/dev_cnt,4) as `30天曝光SKU占比`
		, round(ad60_sku_cnt/dev_cnt,4) as `60天曝光SKU占比`, round(ad90_sku_cnt/dev_cnt,4) as `90天曝光SKU占比`, round(ad120_sku_cnt/dev_cnt,4) as `120天曝光SKU占比`
		, round(ad7_sku_Clicks/ad7_sku_Exposure,4) as `7天点击率`, round(ad14_sku_Clicks/ad14_sku_Exposure,4) as `14天点击率`, round(ad30_sku_Clicks/ad30_sku_Exposure,4) as `30天点击率`
