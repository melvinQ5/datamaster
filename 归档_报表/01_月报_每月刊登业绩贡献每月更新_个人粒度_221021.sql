...
from (
SELECT
	wo.Department, wl.ProductSalesName, wo.shopcode
...
join import_data.wt_listing wl on wo.SellerSku =wl.SellerSKU and wo.shopcode = wl.ShopCode 
where wo.SettlementTime >= '2022-04-01' and wl.PublicationDate >= '2022-04-01'
group by wo.Department, wl.ProductSalesName, wo.shopcode
...
, order_res as ( -- 出单数据
select Department ,ProductSalesName ,pub_month ,set_month 
...
from ords 
group by Department ,ProductSalesName ,pub_month ,set_month 
)

, list_res as ( -- 上架数据
select ws.Department ,ProductSalesName
...
group by ws.Department ,ProductSalesName
	, DATE_FORMAT(wl.PublicationDate,"%Y%m") 
)
...
FROM
    order_res or1
LEFT join list_res lr ON or1.Department = lr.Department AND or1.ProductSalesName = lr.ProductSalesName AND or1.pub_month = lr.pub_month
