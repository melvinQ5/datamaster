-- 取消率
select
	count( distinct case when LateShipmentRateStatus=2  then tmp.ShopCode  end) as warning_shop_cnt -- 最新情况
	...
	,sum(case when ItemType=6 then eaaspcd.Count  end) as LateShipment_ord_cnt -- 滚动累计
	,count( distinct case when ItemType=6 and eaaspcd.Count>0 then tmp.ShopCode  end) as LateShipment_shop_cnt
