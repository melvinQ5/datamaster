...
FROM (select DISTINCT category, pd.OrderNumber, timestampdiff(second, paytime, pd.CreatedTime)/86400 AS gen_days from import_data.OrderDetails od 
	join pd on od.OrderNumber =pd.OrderNumber and TransactionType = '付款'and OrderStatus <> '作废' and OrderTotalPrice > 0	and pd.CreatedTime < '${EndDay}' and pd.CreatedTime >= date_add('${EndDay}',interval -7 day)
...
-- select '所有类目' category,'所有部门' as department, '周报' as ReportType,  weekofyear('${EndDay}') as static_date, '所有产品' as product_tupe, round(avg(cnt),2) `月度SKU采购频次`
-- from (select BoxSku , count(distinct PurchaseOrderNo) cnt from import_data.PurchaseOrder where ordertime >= date_add('${EndDay}',interval -7 day) and ordertime < '${EndDay}' 
-- 	and WarehouseName = '东莞仓' and Monday < '${EndDay}' and Monday >= date_add('${EndDay}',interval -7 day) and ReportType = '周报' group by BoxSku	
-- 	) tmp 
-- union all -- 分类目
-- select category,'所有部门' as department, '周报' as ReportType,  weekofyear('${EndDay}') as static_date, '所有产品' as product_tupe, round(avg(cnt),2) `月度SKU采购频次`
-- from (select BoxSku , count(distinct PurchaseOrderNo) cnt from import_data.PurchaseOrder where ordertime >= date_add('${EndDay}',interval -7 day) and ordertime < '${EndDay}' 
-- 	and WarehouseName = '东莞仓' and Monday < '${EndDay}' and Monday >= date_add('${EndDay}',interval -7 day) and ReportType = '周报' group by BoxSku	
-- 	) tmp left join pt on pt.BoxSKU = tmp.BoxSKU group by category 
-- union all -- 分品类（新品）
-- select '所有类目' category,'所有部门' as department, '周报' as ReportType,  weekofyear('${EndDay}') as static_date, '新品' as product_tupe, round(avg(cnt),2) `月度SKU采购频次`
-- from (select BoxSku , count(distinct PurchaseOrderNo) cnt from import_data.PurchaseOrder where ordertime >= date_add('${EndDay}',interval -7 day) and ordertime < '${EndDay}' 
-- 	and WarehouseName = '东莞仓' and Monday < '${EndDay}' and Monday >= date_add('${EndDay}',interval -7 day) and ReportType = '周报' group by BoxSku	
-- 	) tmp left join pt on pt.BoxSKU = tmp.BoxSKU where is_new=1 
-- union all -- 分品类（重点）
-- select '所有类目' category,'所有部门' as department, '周报' as ReportType, weekofyear('${EndDay}') as static_date, '重点' as product_tupe, round(avg(cnt),2) `月度SKU采购频次`
-- from (select BoxSku , count(distinct PurchaseOrderNo) cnt from import_data.PurchaseOrder where ordertime >= date_add('${EndDay}',interval -7 day) and ordertime < '${EndDay}' 
-- 	and WarehouseName = '东莞仓' and Monday < '${EndDay}' and Monday >= date_add('${EndDay}',interval -7 day) and ReportType = '周报' group by BoxSku	
-- 	) tmp left join pt on pt.BoxSKU = tmp.BoxSKU where is_lead=1 
-- union all -- 分品类（其他）
-- select '所有类目' category,'所有部门' as department, '周报' as ReportType,  weekofyear('${EndDay}') as static_date, '其他' as product_tupe, round(avg(cnt),2) `月度SKU采购频次`
-- from (select BoxSku , count(distinct PurchaseOrderNo) cnt from import_data.PurchaseOrder where ordertime >= date_add('${EndDay}',interval -7 day) and ordertime < '${EndDay}' 
-- 	and WarehouseName = '东莞仓' and Monday < '${EndDay}' and Monday >= date_add('${EndDay}',interval -7 day) and ReportType = '周报' group by BoxSku	
-- 	) tmp left join pt on pt.BoxSKU = tmp.BoxSKU where is_other=1 
-- union all -- 分品类（新品）+分类目
-- select category,'所有部门' as department, '周报' as ReportType,  weekofyear('${EndDay}') as static_date, '新品' as product_tupe, round(avg(cnt),2) `月度SKU采购频次`
-- from (select BoxSku , count(distinct PurchaseOrderNo) cnt from import_data.PurchaseOrder where ordertime >= date_add('${EndDay}',interval -7 day) and ordertime < '${EndDay}' 
-- 	and WarehouseName = '东莞仓' and Monday < '${EndDay}' and Monday >= date_add('${EndDay}',interval -7 day) and ReportType = '周报' group by BoxSku	
-- 	) tmp left join pt on pt.BoxSKU = tmp.BoxSKU where is_new=1 group by category 
-- union all -- 分品类（重点）+分类目
-- select category,'所有部门' as department, '周报' as ReportType,  weekofyear('${EndDay}') as static_date, '重点' as product_tupe, round(avg(cnt),2) `月度SKU采购频次` 
-- from (select BoxSku , count(distinct PurchaseOrderNo) cnt from import_data.PurchaseOrder where ordertime >= date_add('${EndDay}',interval -7 day) and ordertime < '${EndDay}' 
-- 	and WarehouseName = '东莞仓' and Monday < '${EndDay}' and Monday >= date_add('${EndDay}',interval -7 day) and ReportType = '周报' group by BoxSku	
-- 	) tmp left join pt on pt.BoxSKU = tmp.BoxSKU where is_lead=1 group by category 
-- union all -- 分品类（其他）+分类目
-- select category,'所有部门' as department, '周报' as ReportType,  weekofyear('${EndDay}') as static_date, '其他' as product_tupe, round(avg(cnt),2) `月度SKU采购频次`
-- from (select BoxSku , count(distinct PurchaseOrderNo) cnt from import_data.PurchaseOrder where ordertime >= date_add('${EndDay}',interval -7 day) and ordertime < '${EndDay}' 
-- 	and WarehouseName = '东莞仓' and Monday < '${EndDay}' and Monday >= date_add('${EndDay}',interval -7 day) and ReportType = '周报' group by BoxSku	
