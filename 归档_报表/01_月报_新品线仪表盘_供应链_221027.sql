...
where TransactionType = '付款'and OrderStatus <> '作废' and OrderTotalPrice > 0
	and PayTime < '${EndDay}' and PayTime >= date_add('${EndDay}',interval -1 month)
)
...
where WarehouseName = '东莞仓' and Monday < '${EndDay}' and Monday >= date_add('${EndDay}',interval -1 month) and ReportType = '月报'
)
...
from (select * from import_data.PurchaseOrder 
	where ordertime >= date_add('${EndDay}',interval -1 month) and ordertime < '${EndDay}' 
	and WarehouseName = '东莞仓' and Monday < '${EndDay}' and Monday >= date_add('${EndDay}',interval -1 month) and ReportType = '月报' 
	) po
...
select '所有类目' as category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date,'所有产品' as product_tupe
	...
	from po
union all -- 分类目
select category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date,'所有产品' as product_tupe
	, count(distinct(OrderNumber)) `采购单数` , count(distinct case when total_qy <3 then OrderNumber end) `零散采购单数`
	, count(distinct(SupplierId)) `供应商数量`, count(distinct case when supplier_amount>1000 then SupplierId end) `供应商数（供货>1000元)` 
	, sum(case when supplier_amount>1000 then Price - DiscountedPrice end) `对应供货金额`, sum(Price - DiscountedPrice) `总供货金额`
	from po left join pt on pt.BoxSKU = po.BoxSKU group by category 
union all -- 分品类（新品）
select '所有类目' as category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '新品' as product_tupe
	, count(distinct(OrderNumber)) `采购单数` , count(distinct case when total_qy <3 then OrderNumber end) `零散采购单数`
	, count(distinct(SupplierId)) `供应商数量`, count(distinct case when supplier_amount>1000 then SupplierId end) `供应商数（供货>1000元)` 
	, sum(case when supplier_amount>1000 then Price - DiscountedPrice end) `对应供货金额`, sum(Price - DiscountedPrice) `总供货金额`
	from po left join pt on pt.BoxSKU = po.BoxSKU  where is_new=1 
union all -- 分品类（重点）
select '所有类目' as category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '重点' as product_tupe
	, count(distinct(OrderNumber)) `采购单数` , count(distinct case when total_qy <3 then OrderNumber end) `零散采购单数`
	, count(distinct(SupplierId)) `供应商数量`, count(distinct case when supplier_amount>1000 then SupplierId end) `供应商数（供货>1000元)` 
	, sum(case when supplier_amount>1000 then Price - DiscountedPrice end) `对应供货金额`, sum(Price - DiscountedPrice) `总供货金额`
	from po left join pt on pt.BoxSKU = po.BoxSKU  where is_lead=1 
union all -- 分品类（其他）
select '所有类目' as category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '其他' as product_tupe
	, count(distinct(OrderNumber)) `采购单数` , count(distinct case when total_qy <3 then OrderNumber end) `零散采购单数`
	, count(distinct(SupplierId)) `供应商数量`, count(distinct case when supplier_amount>1000 then SupplierId end) `供应商数（供货>1000元)` 
	, sum(case when supplier_amount>1000 then Price - DiscountedPrice end) `对应供货金额`, sum(Price - DiscountedPrice) `总供货金额`
	from po left join pt on pt.BoxSKU = po.BoxSKU  where is_other=1 
union all -- 分品类（新品）+分类目
select category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '新品' as product_tupe
	, count(distinct(OrderNumber)) `采购单数` , count(distinct case when total_qy <3 then OrderNumber end) `零散采购单数`
	, count(distinct(SupplierId)) `供应商数量`, count(distinct case when supplier_amount>1000 then SupplierId end) `供应商数（供货>1000元)` 
	, sum(case when supplier_amount>1000 then Price - DiscountedPrice end) `对应供货金额`, sum(Price - DiscountedPrice) `总供货金额`
	from po left join pt on pt.BoxSKU = po.BoxSKU where is_new=1 group by category 
union all -- 分品类（重点）+分类目
select category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '重点' as product_tupe
	, count(distinct(OrderNumber)) `采购单数` , count(distinct case when total_qy <3 then OrderNumber end) `零散采购单数`
	, count(distinct(SupplierId)) `供应商数量`, count(distinct case when supplier_amount>1000 then SupplierId end) `供应商数（供货>1000元)` 
	, sum(case when supplier_amount>1000 then Price - DiscountedPrice end) `对应供货金额`, sum(Price - DiscountedPrice) `总供货金额`
	from po left join pt on pt.BoxSKU = po.BoxSKU where is_new=1 group by category  
union all -- 分品类（其他）+分类目
select category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '其他' as product_tupe
	, count(distinct(OrderNumber)) `采购单数` , count(distinct case when total_qy <3 then OrderNumber end) `零散采购单数`
	, count(distinct(SupplierId)) `供应商数量`, count(distinct case when supplier_amount>1000 then SupplierId end) `供应商数（供货>1000元)` 
	, sum(case when supplier_amount>1000 then Price - DiscountedPrice end) `对应供货金额`, sum(Price - DiscountedPrice) `总供货金额`
	from po left join pt on pt.BoxSKU = po.BoxSKU where is_new=1 group by category  
...
where date_add(ordertime, 5)  >= date_add('${EndDay}',interval -1 month) and date_add(ordertime, 5) < '${EndDay}' 
	and WarehouseName = '东莞仓' and Monday < '${EndDay}' and Monday >= date_add('${EndDay}',interval -1 month) and ReportType = '月报' 
)

, purchase_arrived_5day_rate as ( -- 采购5天到货率
select '所有类目' as category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date,'所有产品' as product_tupe
...
join import_data.mysql_store s on s.code = a.ShopIrobotId
where TransactionType = '付款'and OrderStatus <> '作废' and OrderTotalPrice > 0
	and PayTime < '${EndDay}' and date_add(PayTime,10) >= date_add('${EndDay}',interval -1 month) -- 再往前预留10天的数据，便于后续计算往前推天数
)
...
from  (select '所有类目' category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '所有产品' as product_tupe
	, count(distinct case when date_add(PayTime, 2) < '${EndDay}' and date_add(PayTime, 2) >= date_add('${EndDay}',interval -1 month) then od_pre.OrderNumber end ) b_gen_in2d -- 2天生包率分母
	, count(distinct case when date_add(PayTime, 5) < '${EndDay}' and date_add(PayTime, 5) >= date_add('${EndDay}',interval -1 month) then od_pre.OrderNumber end ) b_deliv_in5d -- 5天发货率分母
	, count(distinct case when date_add(PayTime, 7) < '${EndDay}' and date_add(PayTime, 7) >= date_add('${EndDay}',interval -1 month) then od_pre.OrderNumber end ) b_deliv_in7d -- 7天发货率分母
	, count(distinct case when date_add(PayTime, 2) < '${EndDay}' and date_add(PayTime, 2) >= date_add('${EndDay}',interval -1 month) and timestampdiff(second, paytime, pd.CreatedTime) <= (86400 * 2) 
		then pd.OrderNumber end ) a_gen_in2d -- 2天生包率分子
	, count(distinct case when date_add(PayTime, 5) < '${EndDay}' and date_add(PayTime, 5) >= date_add('${EndDay}',interval -1 month) and timestampdiff(second, paytime, pd.WeightTIme) <= (86400 * 5) 
		and timestampdiff(second, paytime, pd.WeightTIme) > 0 then pd.OrderNumber end ) a_deliv_in5d -- 5天订单发货率分子
	, count(distinct case when date_add(PayTime, 7) < '${EndDay}' and date_add(PayTime, 7) >= date_add('${EndDay}',interval -1 month) and timestampdiff(second, paytime, pd.WeightTIme) <= (86400 * 7) 
...
union all -- 分类目
select category,department,ReportType,static_date,product_tupe, round(a_gen_in2d/b_gen_in2d,4) `2天生包率`, round(a_deliv_in5d/b_deliv_in5d,4) `订单5天发货率`, round(a_deliv_in7d/b_deliv_in7d,4) `订单7天发货率`
from  (select category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '所有产品' as product_tupe
	, count(distinct case when date_add(PayTime, 2) < '${EndDay}' and date_add(PayTime, 2) >= date_add('${EndDay}',interval -1 month) then od_pre.OrderNumber end ) b_gen_in2d -- 2天生包率分母
	, count(distinct case when date_add(PayTime, 5) < '${EndDay}' and date_add(PayTime, 5) >= date_add('${EndDay}',interval -1 month) then od_pre.OrderNumber end ) b_deliv_in5d -- 5天发货率分母
	, count(distinct case when date_add(PayTime, 7) < '${EndDay}' and date_add(PayTime, 7) >= date_add('${EndDay}',interval -1 month) then od_pre.OrderNumber end ) b_deliv_in7d -- 7天发货率分母
	, count(distinct case when date_add(PayTime, 2) < '${EndDay}' and date_add(PayTime, 2) >= date_add('${EndDay}',interval -1 month) and timestampdiff(second, paytime, pd.CreatedTime) <= (86400 * 2) 
...
union all -- 分品类（新品）
select category,department,ReportType,static_date,product_tupe, round(a_gen_in2d/b_gen_in2d,4) `2天生包率`, round(a_deliv_in5d/b_deliv_in5d,4) `订单5天发货率`, round(a_deliv_in7d/b_deliv_in7d,4) `订单7天发货率`
from  (select '所有类目' category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '新品' as product_tupe
	, count(distinct case when date_add(PayTime, 2) < '${EndDay}' and date_add(PayTime, 2) >= date_add('${EndDay}',interval -1 month) then od_pre.OrderNumber end ) b_gen_in2d -- 2天生包率分母
	, count(distinct case when date_add(PayTime, 5) < '${EndDay}' and date_add(PayTime, 5) >= date_add('${EndDay}',interval -1 month) then od_pre.OrderNumber end ) b_deliv_in5d -- 5天发货率分母
	, count(distinct case when date_add(PayTime, 7) < '${EndDay}' and date_add(PayTime, 7) >= date_add('${EndDay}',interval -1 month) then od_pre.OrderNumber end ) b_deliv_in7d -- 7天发货率分母
	, count(distinct case when date_add(PayTime, 2) < '${EndDay}' and date_add(PayTime, 2) >= date_add('${EndDay}',interval -1 month) and timestampdiff(second, paytime, pd.CreatedTime) <= (86400 * 2) 
...
union all -- 分品类（重点）
select category,department,ReportType,static_date,product_tupe, round(a_gen_in2d/b_gen_in2d,4) `2天生包率`, round(a_deliv_in5d/b_deliv_in5d,4) `订单5天发货率`, round(a_deliv_in7d/b_deliv_in7d,4) `订单7天发货率`
from  (select '所有类目' category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '重点' as product_tupe
	, count(distinct case when date_add(PayTime, 2) < '${EndDay}' and date_add(PayTime, 2) >= date_add('${EndDay}',interval -1 month) then od_pre.OrderNumber end ) b_gen_in2d -- 2天生包率分母
	, count(distinct case when date_add(PayTime, 5) < '${EndDay}' and date_add(PayTime, 5) >= date_add('${EndDay}',interval -1 month) then od_pre.OrderNumber end ) b_deliv_in5d -- 5天发货率分母
	, count(distinct case when date_add(PayTime, 7) < '${EndDay}' and date_add(PayTime, 7) >= date_add('${EndDay}',interval -1 month) then od_pre.OrderNumber end ) b_deliv_in7d -- 7天发货率分母
	, count(distinct case when date_add(PayTime, 2) < '${EndDay}' and date_add(PayTime, 2) >= date_add('${EndDay}',interval -1 month) and timestampdiff(second, paytime, pd.CreatedTime) <= (86400 * 2) 
...
union all -- 分品类（其他）
select category,department,ReportType,static_date,product_tupe, round(a_gen_in2d/b_gen_in2d,4) `2天生包率`, round(a_deliv_in5d/b_deliv_in5d,4) `订单5天发货率`, round(a_deliv_in7d/b_deliv_in7d,4) `订单7天发货率`
from  (select '所有类目' category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '其他' as product_tupe
	, count(distinct case when date_add(PayTime, 2) < '${EndDay}' and date_add(PayTime, 2) >= date_add('${EndDay}',interval -1 month) then od_pre.OrderNumber end ) b_gen_in2d -- 2天生包率分母
	, count(distinct case when date_add(PayTime, 5) < '${EndDay}' and date_add(PayTime, 5) >= date_add('${EndDay}',interval -1 month) then od_pre.OrderNumber end ) b_deliv_in5d -- 5天发货率分母
	, count(distinct case when date_add(PayTime, 7) < '${EndDay}' and date_add(PayTime, 7) >= date_add('${EndDay}',interval -1 month) then od_pre.OrderNumber end ) b_deliv_in7d -- 7天发货率分母
	, count(distinct case when date_add(PayTime, 2) < '${EndDay}' and date_add(PayTime, 2) >= date_add('${EndDay}',interval -1 month) and timestampdiff(second, paytime, pd.CreatedTime) <= (86400 * 2) 
...
union all -- 分品类（新品）+分类目
select category,department,ReportType,static_date,product_tupe, round(a_gen_in2d/b_gen_in2d,4) `2天生包率`, round(a_deliv_in5d/b_deliv_in5d,4) `订单5天发货率`, round(a_deliv_in7d/b_deliv_in7d,4) `订单7天发货率`
from  (select category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '新品' as product_tupe
	, count(distinct case when date_add(PayTime, 2) < '${EndDay}' and date_add(PayTime, 2) >= date_add('${EndDay}',interval -1 month) then od_pre.OrderNumber end ) b_gen_in2d -- 2天生包率分母
	, count(distinct case when date_add(PayTime, 5) < '${EndDay}' and date_add(PayTime, 5) >= date_add('${EndDay}',interval -1 month) then od_pre.OrderNumber end ) b_deliv_in5d -- 5天发货率分母
	, count(distinct case when date_add(PayTime, 7) < '${EndDay}' and date_add(PayTime, 7) >= date_add('${EndDay}',interval -1 month) then od_pre.OrderNumber end ) b_deliv_in7d -- 7天发货率分母
	, count(distinct case when date_add(PayTime, 2) < '${EndDay}' and date_add(PayTime, 2) >= date_add('${EndDay}',interval -1 month) and timestampdiff(second, paytime, pd.CreatedTime) <= (86400 * 2) 
...
union all -- 分品类（重点）+分类目
select category,department,ReportType,static_date,product_tupe, round(a_gen_in2d/b_gen_in2d,4) `2天生包率`, round(a_deliv_in5d/b_deliv_in5d,4) `订单5天发货率`, round(a_deliv_in7d/b_deliv_in7d,4) `订单7天发货率`
from  (select category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '重点' as product_tupe
	, count(distinct case when date_add(PayTime, 2) < '${EndDay}' and date_add(PayTime, 2) >= date_add('${EndDay}',interval -1 month) then od_pre.OrderNumber end ) b_gen_in2d -- 2天生包率分母
	, count(distinct case when date_add(PayTime, 5) < '${EndDay}' and date_add(PayTime, 5) >= date_add('${EndDay}',interval -1 month) then od_pre.OrderNumber end ) b_deliv_in5d -- 5天发货率分母
	, count(distinct case when date_add(PayTime, 7) < '${EndDay}' and date_add(PayTime, 7) >= date_add('${EndDay}',interval -1 month) then od_pre.OrderNumber end ) b_deliv_in7d -- 7天发货率分母
	, count(distinct case when date_add(PayTime, 2) < '${EndDay}' and date_add(PayTime, 2) >= date_add('${EndDay}',interval -1 month) and timestampdiff(second, paytime, pd.CreatedTime) <= (86400 * 2) 
...
union all -- 分品类（其他）+分类目
select category,department,ReportType,static_date,product_tupe, round(a_gen_in2d/b_gen_in2d,4) `2天生包率`, round(a_deliv_in5d/b_deliv_in5d,4) `订单5天发货率`, round(a_deliv_in7d/b_deliv_in7d,4) `订单7天发货率`
from  (select category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '其他' as product_tupe
	, count(distinct case when date_add(PayTime, 2) < '${EndDay}' and date_add(PayTime, 2) >= date_add('${EndDay}',interval -1 month) then od_pre.OrderNumber end ) b_gen_in2d -- 2天生包率分母
	, count(distinct case when date_add(PayTime, 5) < '${EndDay}' and date_add(PayTime, 5) >= date_add('${EndDay}',interval -1 month) then od_pre.OrderNumber end ) b_deliv_in5d -- 5天发货率分母
	, count(distinct case when date_add(PayTime, 7) < '${EndDay}' and date_add(PayTime, 7) >= date_add('${EndDay}',interval -1 month) then od_pre.OrderNumber end ) b_deliv_in7d -- 7天发货率分母
	, count(distinct case when date_add(PayTime, 2) < '${EndDay}' and date_add(PayTime, 2) >= date_add('${EndDay}',interval -1 month) and timestampdiff(second, paytime, pd.CreatedTime) <= (86400 * 2) 
...
, avg_gen_day AS ( -- 平均生包天数，统计上周所有生成的包裹，不限制客户支付时间
select '所有类目' category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '所有产品' as product_tupe
...
FROM (select DISTINCT pd.OrderNumber, timestampdiff(second, paytime, pd.CreatedTime)/86400 AS gen_days from import_data.OrderDetails od 
	join pd on od.OrderNumber =pd.OrderNumber and TransactionType = '付款'and OrderStatus <> '作废' and OrderTotalPrice > 0	
		and pd.CreatedTime < '${EndDay}' and pd.CreatedTime >= date_add('${EndDay}',interval -1 month)) tmp
union all -- 分类目
select category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '所有产品' as product_tupe
	, sum(gen_days)/count(DISTINCT OrderNumber) `平均生包天数`
FROM (select DISTINCT category, pd.OrderNumber, timestampdiff(second, paytime, pd.CreatedTime)/86400 AS gen_days from import_data.OrderDetails od 
	join pd on od.OrderNumber =pd.OrderNumber and TransactionType = '付款'and OrderStatus <> '作废' and OrderTotalPrice > 0	and pd.CreatedTime < '${EndDay}' and pd.CreatedTime >= date_add('${EndDay}',interval -1 month)
	left join pt on pt.BoxSKU = od.BoxSku ) tmp group by category
union all -- 分品类（新品）
select '所有类目' category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '新品' as product_tupe
	, sum(gen_days)/count(DISTINCT OrderNumber) `平均生包天数`
FROM (select DISTINCT pd.OrderNumber, timestampdiff(second, paytime, pd.CreatedTime)/86400 AS gen_days from import_data.OrderDetails od 
	join pd on od.OrderNumber =pd.OrderNumber and TransactionType = '付款'and OrderStatus <> '作废' and OrderTotalPrice > 0	and pd.CreatedTime < '${EndDay}' and pd.CreatedTime >= date_add('${EndDay}',interval -1 month)
	left join pt on pt.BoxSKU = od.BoxSku  WHERE is_new=1  ) tmp 
union all -- 分品类（重点）
select '所有类目' category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '重点' as product_tupe
	, sum(gen_days)/count(DISTINCT OrderNumber) `平均生包天数`
FROM (select DISTINCT pd.OrderNumber, timestampdiff(second, paytime, pd.CreatedTime)/86400 AS gen_days from import_data.OrderDetails od 
	join pd on od.OrderNumber =pd.OrderNumber and TransactionType = '付款'and OrderStatus <> '作废' and OrderTotalPrice > 0	and pd.CreatedTime < '${EndDay}' and pd.CreatedTime >= date_add('${EndDay}',interval -1 month)
	left join pt on pt.BoxSKU = od.BoxSku  WHERE is_lead=1  ) tmp 
union all -- 分品类（其他）
select '所有类目' category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '其他' as product_tupe
	, sum(gen_days)/count(DISTINCT OrderNumber) `平均生包天数`
FROM (select DISTINCT pd.OrderNumber, timestampdiff(second, paytime, pd.CreatedTime)/86400 AS gen_days from import_data.OrderDetails od 
	join pd on od.OrderNumber =pd.OrderNumber and TransactionType = '付款'and OrderStatus <> '作废' and OrderTotalPrice > 0	and pd.CreatedTime < '${EndDay}' and pd.CreatedTime >= date_add('${EndDay}',interval -1 month)
...
, avg_delivery_day AS ( -- 平均发货天数，统计上周所有发货的包裹，不限制客户支付时间
select '所有类目' category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '所有产品' as product_tupe
...
from import_data.OrderDetails od  join pd on od.OrderNumber =pd.OrderNumber and TransactionType = '付款'and OrderStatus <> '作废' and OrderTotalPrice > 0
	WHERE weighttIme < '${EndDay}' and weighttIme >= date_add('${EndDay}',interval -1 month) ) tmp
union all -- 分类目
select category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '所有产品' as product_tupe
	, sum(deli_days)/count(DISTINCT OrderNumber) `平均发货天数`
FROM (select DISTINCT category, pd.OrderNumber, timestampdiff(second, paytime, pd.weighttIme)/86400 AS deli_days from import_data.OrderDetails od  
	join pd on od.OrderNumber =pd.OrderNumber and TransactionType = '付款'and OrderStatus <> '作废' and OrderTotalPrice > 0
	left join pt on pt.BoxSKU = od.BoxSku WHERE weighttIme < '${EndDay}' and weighttIme >= date_add('${EndDay}',interval -1 month) ) tmp group by category
union all -- 分品类（新品）
select '所有类目' category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '新品' as product_tupe
	, sum(deli_days)/count(DISTINCT OrderNumber) `平均发货天数`
FROM (select DISTINCT pd.OrderNumber, timestampdiff(second, paytime, pd.weighttIme)/86400 AS deli_days from import_data.OrderDetails od  
	join pd on od.OrderNumber =pd.OrderNumber and TransactionType = '付款'and OrderStatus <> '作废' and OrderTotalPrice > 0
	left join pt on pt.BoxSKU = od.BoxSku  WHERE is_new=1 AND weighttIme < '${EndDay}' and weighttIme >= date_add('${EndDay}',interval -1 month)  ) tmp 
union all -- 分品类（重点）
select '所有类目' category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '重点' as product_tupe
	, sum(deli_days)/count(DISTINCT OrderNumber) `平均发货天数`
FROM (select DISTINCT pd.OrderNumber, timestampdiff(second, paytime, pd.weighttIme)/86400 AS deli_days from import_data.OrderDetails od  
	join pd on od.OrderNumber =pd.OrderNumber and TransactionType = '付款'and OrderStatus <> '作废' and OrderTotalPrice > 0	
	left join pt on pt.BoxSKU = od.BoxSku  WHERE is_lead=1 AND weighttIme < '${EndDay}' and weighttIme >= date_add('${EndDay}',interval -1 month)  ) tmp 
union all -- 分品类（其他）
select '所有类目' category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '其他' as product_tupe
	,  sum(deli_days)/count(DISTINCT OrderNumber) `平均发货天数`
FROM (select DISTINCT pd.OrderNumber, timestampdiff(second, paytime, pd.weighttIme)/86400 AS deli_days from import_data.OrderDetails od  
	join pd on od.OrderNumber =pd.OrderNumber and TransactionType = '付款'and OrderStatus <> '作废' and OrderTotalPrice > 0	
	left join pt on pt.BoxSKU = od.BoxSku  WHERE is_other=1 AND weighttIme < '${EndDay}' and weighttIme >= date_add('${EndDay}',interval -1 month) ) tmp 
union all -- 分品类（新品）+分类目
select category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '新品' as product_tupe
	,  sum(deli_days)/count(DISTINCT OrderNumber) `平均发货天数`
FROM (select DISTINCT category, pd.OrderNumber, timestampdiff(second, paytime, pd.weighttIme)/86400 AS deli_days from import_data.OrderDetails od  
	join pd on od.OrderNumber =pd.OrderNumber and TransactionType = '付款'and OrderStatus <> '作废' and OrderTotalPrice > 0	
	left join pt on pt.BoxSKU = od.BoxSku  WHERE is_new=1 AND weighttIme < '${EndDay}' and weighttIme >= date_add('${EndDay}',interval -1 month) ) tmp group by category
union all -- 分品类（重点）+分类目
select category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '重点' as product_tupe
	,  sum(deli_days)/count(DISTINCT OrderNumber) `平均发货天数`
FROM (select DISTINCT category, pd.OrderNumber, timestampdiff(second, paytime, pd.weighttIme)/86400 AS deli_days from import_data.OrderDetails od  
	join pd on od.OrderNumber =pd.OrderNumber and TransactionType = '付款'and OrderStatus <> '作废' and OrderTotalPrice > 0	
	left join pt on pt.BoxSKU = od.BoxSku  WHERE is_lead=1 AND weighttIme < '${EndDay}' and weighttIme >= date_add('${EndDay}',interval -1 month) ) tmp group by category
union all -- 分品类（其他）+分类目
select category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '其他' as product_tupe
	,  sum(deli_days)/count(DISTINCT OrderNumber) `平均发货天数`
FROM (select DISTINCT category, pd.OrderNumber, timestampdiff(second, paytime, pd.weighttIme)/86400 AS deli_days from import_data.OrderDetails od  
	join pd on od.OrderNumber =pd.OrderNumber and TransactionType = '付款'and OrderStatus <> '作废' and OrderTotalPrice > 0	
	left join pt on pt.BoxSKU = od.BoxSku  WHERE is_other=1 AND weighttIme < '${EndDay}' and weighttIme >= date_add('${EndDay}',interval -1 month)  ) tmp group by category
)
...
from pd where CreatedTime < '${EndDay}' and CreatedTime >= date_add('${EndDay}',interval -1 month) 
union all -- 分类目
select category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '所有产品' as product_tupe
	, round(count(case when timestampdiff(second , CreatedTime, WeightTime) <= 86400 and timestampdiff(second , CreatedTime, WeightTime) > 0 then 1 end)/count(1),4) `24小时发货率`
from pd left join pt on pt.BoxSKU = pd.BoxSKU where CreatedTime < '${EndDay}' and CreatedTime >= date_add('${EndDay}',interval -1 month) group by category
union all -- 分品类（新品）
select '所有类目' category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '新品' as product_tupe
	, round(count(case when timestampdiff(second , CreatedTime, WeightTime) <= 86400 and timestampdiff(second , CreatedTime, WeightTime) > 0 then 1 end)/count(1),4) `24小时发货率`
from pd left join pt on pt.BoxSKU = pd.BoxSKU where CreatedTime < '${EndDay}' and CreatedTime >= date_add('${EndDay}',interval -1 month)  and is_new=1 
union all -- 分品类（重点）
select '所有类目' category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '重点' as product_tupe
	, round(count(case when timestampdiff(second , CreatedTime, WeightTime) <= 86400 and timestampdiff(second , CreatedTime, WeightTime) > 0 then 1 end)/count(1),4) `24小时发货率`
from pd left join pt on pt.BoxSKU = pd.BoxSKU where CreatedTime < '${EndDay}' and CreatedTime >= date_add('${EndDay}',interval -1 month)  and is_lead=1 
union all -- 分品类（其他）
select '所有类目' category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '其他' as product_tupe
	, round(count(case when timestampdiff(second , CreatedTime, WeightTime) <= 86400 and timestampdiff(second , CreatedTime, WeightTime) > 0 then 1 end)/count(1),4) `24小时发货率`
from pd left join pt on pt.BoxSKU = pd.BoxSKU where CreatedTime < '${EndDay}' and CreatedTime >= date_add('${EndDay}',interval -1 month)  and is_other=1 
union all -- 分品类（新品）+分类目
select category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '新品' as product_tupe
	, round(count(case when timestampdiff(second , CreatedTime, WeightTime) <= 86400 and timestampdiff(second , CreatedTime, WeightTime) > 0 then 1 end)/count(1),4) `24小时发货率`
from pd left join pt on pt.BoxSKU = pd.BoxSKU where CreatedTime < '${EndDay}' and CreatedTime >= date_add('${EndDay}',interval -1 month) and is_new=1 group by category
union all -- 分品类（重点）+分类目
select category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '重点' as product_tupe
	, round(count(case when timestampdiff(second , CreatedTime, WeightTime) <= 86400 and timestampdiff(second , CreatedTime, WeightTime) > 0 then 1 end)/count(1),4) `24小时发货率`
from pd left join pt on pt.BoxSKU = pd.BoxSKU where CreatedTime < '${EndDay}' and CreatedTime >= date_add('${EndDay}',interval -1 month) and is_lead=1 group by category
union all -- 分品类（其他）+分类目
select category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '其他' as product_tupe
	, round(count(case when timestampdiff(second , CreatedTime, WeightTime) <= 86400 and timestampdiff(second , CreatedTime, WeightTime) > 0 then 1 end)/count(1),4) `24小时发货率`
from pd left join pt on pt.BoxSKU = pd.BoxSKU where CreatedTime < '${EndDay}' and CreatedTime >= date_add('${EndDay}',interval -1 month) and is_other=1 group by category
)
-- 
...
where date_add(scantime, 2) < '${EndDay}' and date_add(scantime, 2) >= date_add('${EndDay}',interval -1 month) 
and Monday < '${EndDay}' and Monday >= date_add('${EndDay}',interval -1 month) and ReportType = '月报' and b.WarehouseName = '东莞仓'
union all -- 分类目
select category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '所有产品' as product_tupe
	, round(count(distinct case when timestampdiff(second, ScanTime, CompleteTime) <= (2 * 86400) then a.PurchaseOrderNo end )/count(distinct a.PurchaseOrderNo),4) `48小时收货率`
...
where date_add(scantime, 2) < '${EndDay}' and date_add(scantime, 2) >= date_add('${EndDay}',interval -1 month) 
	and Monday < '${EndDay}' and Monday >= date_add('${EndDay}',interval -1 month) and ReportType = '月报' and b.WarehouseName = '东莞仓' group by category
union all -- 分品类（新品）
select '所有类目' category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '新品' as product_tupe
	, round(count(distinct case when timestampdiff(second, ScanTime, CompleteTime) <= (2 * 86400) then a.PurchaseOrderNo end )/count(distinct a.PurchaseOrderNo),4) `48小时收货率`
from import_data.PurchaseRev a join import_data.PurchaseOrder b on  a.PurchaseOrderNo = b.PurchaseOrderNo left join pt on pt.BoxSKU = b.BoxSKU
where date_add(scantime, 2) < '${EndDay}' and date_add(scantime, 2) >= date_add('${EndDay}',interval -1 month) 
	and Monday < '${EndDay}' and Monday >= date_add('${EndDay}',interval -1 month) and ReportType = '月报' and b.WarehouseName = '东莞仓' and is_new=1 
union all -- 分品类（重点）
select '所有类目' category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '重点' as product_tupe
	, round(count(distinct case when timestampdiff(second, ScanTime, CompleteTime) <= (2 * 86400) then a.PurchaseOrderNo end )/count(distinct a.PurchaseOrderNo),4) `48小时收货率`
from import_data.PurchaseRev a join import_data.PurchaseOrder b on  a.PurchaseOrderNo = b.PurchaseOrderNo left join pt on pt.BoxSKU = b.BoxSKU
where date_add(scantime, 2) < '${EndDay}' and date_add(scantime, 2) >= date_add('${EndDay}',interval -1 month) 
	and Monday < '${EndDay}' and Monday >= date_add('${EndDay}',interval -1 month) and ReportType = '月报' and b.WarehouseName = '东莞仓' and is_lead=1 
union all -- 分品类（其他）
select '所有类目' category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '其他' as product_tupe
	, round(count(distinct case when timestampdiff(second, ScanTime, CompleteTime) <= (2 * 86400) then a.PurchaseOrderNo end )/count(distinct a.PurchaseOrderNo),4) `48小时收货率`
from import_data.PurchaseRev a join import_data.PurchaseOrder b on  a.PurchaseOrderNo = b.PurchaseOrderNo left join pt on pt.BoxSKU = b.BoxSKU
where date_add(scantime, 2) < '${EndDay}' and date_add(scantime, 2) >= date_add('${EndDay}',interval -1 month) 
	and Monday < '${EndDay}' and Monday >= date_add('${EndDay}',interval -1 month) and ReportType = '月报' and b.WarehouseName = '东莞仓' and is_other=1 
union all -- 分品类（新品）+分类目
select category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '新品' as product_tupe
	, round(count(distinct case when timestampdiff(second, ScanTime, CompleteTime) <= (2 * 86400) then a.PurchaseOrderNo end )/count(distinct a.PurchaseOrderNo),4) `48小时收货率`
from import_data.PurchaseRev a join import_data.PurchaseOrder b on  a.PurchaseOrderNo = b.PurchaseOrderNo left join pt on pt.BoxSKU = b.BoxSKU
where date_add(scantime, 2) < '${EndDay}' and date_add(scantime, 2) >= date_add('${EndDay}',interval -1 month) 
	and Monday < '${EndDay}' and Monday >= date_add('${EndDay}',interval -1 month) and ReportType = '月报' and b.WarehouseName = '东莞仓' and is_new=1 group by category
union all -- 分品类（重点）+分类目
select category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '重点' as product_tupe
	, round(count(distinct case when timestampdiff(second, ScanTime, CompleteTime) <= (2 * 86400) then a.PurchaseOrderNo end )/count(distinct a.PurchaseOrderNo),4) `48小时收货率`
from import_data.PurchaseRev a join import_data.PurchaseOrder b on  a.PurchaseOrderNo = b.PurchaseOrderNo left join pt on pt.BoxSKU = b.BoxSKU
where date_add(scantime, 2) < '${EndDay}' and date_add(scantime, 2) >= date_add('${EndDay}',interval -1 month) 
	and Monday < '${EndDay}' and Monday >= date_add('${EndDay}',interval -1 month) and ReportType = '月报' and b.WarehouseName = '东莞仓' and is_lead=1 group by category
union all -- 分品类（其他）+分类目
select category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '其他' as product_tupe
	, round(count(distinct case when timestampdiff(second, ScanTime, CompleteTime) <= (2 * 86400) then a.PurchaseOrderNo end )/count(distinct a.PurchaseOrderNo),4) `48小时收货率`
from import_data.PurchaseRev a join import_data.PurchaseOrder b on  a.PurchaseOrderNo = b.PurchaseOrderNo left join pt on pt.BoxSKU = b.BoxSKU
where date_add(scantime, 2) < '${EndDay}' and date_add(scantime, 2) >= date_add('${EndDay}',interval -1 month) 
	and Monday < '${EndDay}' and Monday >= date_add('${EndDay}',interval -1 month) and ReportType = '月报' and b.WarehouseName = '东莞仓' and is_other=1 group by category
) 

-- 月度计算
, sku_purchase_times as (-- SKU月度采购频次=近4周sku下采购单次数），采购频次高的SKU，考虑优化提高单次采购量
select '所有类目' category,'所有部门' as department, '月报' as ReportType,  month(date_add('${EndDay}',interval -1 month)) as static_date, '所有产品' as product_tupe, round(avg(cnt),2) `月度SKU采购频次`
...
union all -- 分类目
select category,'所有部门' as department, '月报' as ReportType,  month(date_add('${EndDay}',interval -1 month)) as static_date, '所有产品' as product_tupe, round(avg(cnt),2) `月度SKU采购频次`
from (select BoxSku , count(distinct PurchaseOrderNo) cnt from import_data.PurchaseOrder where ordertime >= date_add('${EndDay}',interval -1 month) and ordertime < '${EndDay}' 
	and WarehouseName = '东莞仓' and Monday < '${EndDay}' and Monday >= date_add('${EndDay}',interval -1 month) and ReportType = '月报' group by BoxSku	
	) tmp left join pt on pt.BoxSKU = tmp.BoxSKU group by category 
union all -- 分品类（新品）
select '所有类目' category,'所有部门' as department, '月报' as ReportType,  month(date_add('${EndDay}',interval -1 month)) as static_date, '新品' as product_tupe, round(avg(cnt),2) `月度SKU采购频次`
from (select BoxSku , count(distinct PurchaseOrderNo) cnt from import_data.PurchaseOrder where ordertime >= date_add('${EndDay}',interval -1 month) and ordertime < '${EndDay}' 
	and WarehouseName = '东莞仓' and Monday < '${EndDay}' and Monday >= date_add('${EndDay}',interval -1 month) and ReportType = '月报' group by BoxSku	
	) tmp left join pt on pt.BoxSKU = tmp.BoxSKU where is_new=1 
union all -- 分品类（重点）
select '所有类目' category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '重点' as product_tupe, round(avg(cnt),2) `月度SKU采购频次`
from (select BoxSku , count(distinct PurchaseOrderNo) cnt from import_data.PurchaseOrder where ordertime >= date_add('${EndDay}',interval -1 month) and ordertime < '${EndDay}' 
	and WarehouseName = '东莞仓' and Monday < '${EndDay}' and Monday >= date_add('${EndDay}',interval -1 month) and ReportType = '月报' group by BoxSku	
	) tmp left join pt on pt.BoxSKU = tmp.BoxSKU where is_lead=1 
union all -- 分品类（其他）
select '所有类目' category,'所有部门' as department, '月报' as ReportType,  month(date_add('${EndDay}',interval -1 month)) as static_date, '其他' as product_tupe, round(avg(cnt),2) `月度SKU采购频次`
from (select BoxSku , count(distinct PurchaseOrderNo) cnt from import_data.PurchaseOrder where ordertime >= date_add('${EndDay}',interval -1 month) and ordertime < '${EndDay}' 
	and WarehouseName = '东莞仓' and Monday < '${EndDay}' and Monday >= date_add('${EndDay}',interval -1 month) and ReportType = '月报' group by BoxSku	
	) tmp left join pt on pt.BoxSKU = tmp.BoxSKU where is_other=1 
union all -- 分品类（新品）+分类目
select category,'所有部门' as department, '月报' as ReportType,  month(date_add('${EndDay}',interval -1 month)) as static_date, '新品' as product_tupe, round(avg(cnt),2) `月度SKU采购频次`
from (select BoxSku , count(distinct PurchaseOrderNo) cnt from import_data.PurchaseOrder where ordertime >= date_add('${EndDay}',interval -1 month) and ordertime < '${EndDay}' 
	and WarehouseName = '东莞仓' and Monday < '${EndDay}' and Monday >= date_add('${EndDay}',interval -1 month) and ReportType = '月报' group by BoxSku	
	) tmp left join pt on pt.BoxSKU = tmp.BoxSKU where is_new=1 group by category 
union all -- 分品类（重点）+分类目
select category,'所有部门' as department, '月报' as ReportType,  month(date_add('${EndDay}',interval -1 month)) as static_date, '重点' as product_tupe, round(avg(cnt),2) `月度SKU采购频次` 
from (select BoxSku , count(distinct PurchaseOrderNo) cnt from import_data.PurchaseOrder where ordertime >= date_add('${EndDay}',interval -1 month) and ordertime < '${EndDay}' 
	and WarehouseName = '东莞仓' and Monday < '${EndDay}' and Monday >= date_add('${EndDay}',interval -1 month) and ReportType = '月报' group by BoxSku	
	) tmp left join pt on pt.BoxSKU = tmp.BoxSKU where is_lead=1 group by category 
union all -- 分品类（其他）+分类目
select category,'所有部门' as department, '月报' as ReportType,  month(date_add('${EndDay}',interval -1 month)) as static_date, '其他' as product_tupe, round(avg(cnt),2) `月度SKU采购频次`
from (select BoxSku , count(distinct PurchaseOrderNo) cnt from import_data.PurchaseOrder where ordertime >= date_add('${EndDay}',interval -1 month) and ordertime < '${EndDay}' 
	and WarehouseName = '东莞仓' and Monday < '${EndDay}' and Monday >= date_add('${EndDay}',interval -1 month) and ReportType = '月报' group by BoxSku	
...
, po_Freight as ( -- `采购运费`
select '所有类目' as category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date,'所有产品' as product_tupe, ifnull(sum(fr),0) `采购运费` 
from (select distinct(PurchaseOrderNo), Freight fr from po where IsComplete = '否' and InstockQuantity = 0 ) a 
union all -- 分类目
select category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date,'所有产品' as product_tupe, ifnull(sum(fr),0) `采购运费` 
...
group by category 
union all -- 分品类(新品)
select '所有类目' as category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '新品' as product_tupe, ifnull(sum(fr),0) `采购运费` 
...
union all -- 分品类(重点)
select '所有类目' as category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '重点' as product_tupe, ifnull(sum(fr),0) `采购运费` 
...
union all -- 分品类(其他)
select '所有类目' as category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '其他' as product_tupe, ifnull(sum(fr),0) `采购运费` 
...
union all -- 分品类(新品)+分类目
select category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '新品' as product_tupe, ifnull(sum(fr),0) `采购运费` 
...
group by category 
union all -- 分品类(重点)+分类目
select category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '重点' as product_tupe, ifnull(sum(fr),0) `采购运费` 
...
group by category 
union all -- 分品类(其他)+分类目
select category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, '其他' as product_tupe, ifnull(sum(fr),0) `采购运费` 
...
from ( select distinct(pd.OrderNumber), abs(od.PurchaseCosts/ExchangeUSD) pc 
	...
	where pd.weighttime < '${EndDay}' and pd.weighttime >= date_add('${EndDay}',interval -1 month) ) a  
union all -- 分类目
select category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date,'所有产品' as product_tupe, round(sum(pc)) `发货订单采购金额usd` 
from ( select distinct pd.OrderNumber,od.BoxSku,abs(od.PurchaseCosts/ExchangeUSD) pc 
	from import_data.PackageDetail pd join import_data.OrderDetails od on od.OrderNumber = pd.OrderNumber and od.BoxSku = pd.BoxSku 
	where pd.weighttime < '${EndDay}' and pd.weighttime >= date_add('${EndDay}',interval -1 month)) od 
left join pt on od.BoxSku = pt.BoxSKU group by category
union all -- 分品类(新品)
select '所有类目' as category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date,'新品' as product_tupe
	, round(sum(pc)) `发货订单采购金额usd` 
from ( select distinct pd.OrderNumber,od.BoxSku,abs(od.PurchaseCosts/ExchangeUSD) pc from import_data.PackageDetail pd join import_data.OrderDetails od on od.OrderNumber = pd.OrderNumber and od.BoxSku = pd.BoxSku 
	where pd.weighttime < '${EndDay}' and pd.weighttime >= date_add('${EndDay}',interval -1 month)) od 
...
union all -- 分品类
SELECT '所有类目' as category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, product_tupe
...
union all -- 分品类+分类目
SELECT category,'所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date, product_tupe
	, sum(TotalPrice) `在仓产品金额`, sum(TotalInventory) `在仓sku件数`, count(*) `在仓sku数`
	from (select *, '新品' as product_tupe from lw where is_new = 1 
		union all select *, '重点' as product_tupe from lw where is_lead = 1 
		union all select *, '其他' as product_tupe from lw where is_other = 1) tmp
...
from ( SELECT lw.BoxSKU , count(1) cnt  FROM import_data.erp_amazon_amazon_listing al join lw on lw.ProductId = al.ProductId and lw.is_order = 0
	where PublicationDate < '${EndDay}' and PublicationDate >= date_add('${EndDay}',interval -1 month) and ListingStatus = 1
...
SELECT '所有类目' as category, '所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date,'新品' as product_tupe
	...
	, count(distinct case when cnt > 10 then BoxSKU end) `本周刊登>10条的未出单SKU数`
from ( SELECT lw.category, lw.BoxSKU , count(1) cnt  FROM import_data.erp_amazon_amazon_listing al join lw on lw.ProductId = al.ProductId and lw.is_order = 0
	where PublicationDate < '${EndDay}' and PublicationDate >= date_add('${EndDay}',interval -1 month) and ListingStatus = 1 and lw.is_new = 1 
	group by lw.BoxSKU, lw.category) tmp 
union all -- 分品类(重点)
SELECT '所有类目' as category, '所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date,'重点' as product_tupe
	, count(distinct BoxSKU ) `本周未出单SKU本周刊登SKU数`
	, count(distinct case when cnt > 10 then BoxSKU end) `本周刊登>10条的未出单SKU数`
from ( SELECT lw.category, lw.BoxSKU , count(1) cnt  FROM import_data.erp_amazon_amazon_listing al join lw on lw.ProductId = al.ProductId and lw.is_order = 0
	where PublicationDate < '${EndDay}' and PublicationDate >= date_add('${EndDay}',interval -1 month) and ListingStatus = 1 and lw.is_lead = 1 
	group by lw.BoxSKU, lw.category) tmp 
union all -- 分品类(其他)
SELECT '所有类目' as category, '所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date,'其他' as product_tupe
	, count(distinct BoxSKU ) `本周未出单SKU本周刊登SKU数`
	, count(distinct case when cnt > 10 then BoxSKU end) `本周刊登>10条的未出单SKU数`
from ( SELECT lw.category, lw.BoxSKU , count(1) cnt  FROM import_data.erp_amazon_amazon_listing al join lw on lw.ProductId = al.ProductId and lw.is_order = 0
	where PublicationDate < '${EndDay}' and PublicationDate >= date_add('${EndDay}',interval -1 month) and ListingStatus = 1 and lw.is_other = 1 
	group by lw.BoxSKU, lw.category) tmp
union all -- 分品类(新品)+分类目
SELECT category, '所有部门' as department, '月报' as ReportType, month(date_add('${EndDay}',interval -1 month)) as static_date,'新品' as product_tupe
	, count(distinct BoxSKU ) `本周未出单SKU本周刊登SKU数`
...
select category, department, ReportType, static_date, product_tupe
	, `订单5天发货率`, `订单7天发货率`, `2天生包率`, `采购5天到货率`, `48小时收货率`, `24小时发货率`, `平均生包天数`, `平均发货天数`, `采购单数`, `零散采购单数`, `供应商数量`, `供应商数（供货>1000元)`
	...
	, round((`在仓产品金额`+`采购产品金额`+`采购运费`)/usdratio/`发货订单采购金额usd`*day(date_add('${EndDay}',interval -1 day)))  as `本地仓库存周转天数`
...
from metric_set m,(select usdratio from import_data.Basedata where firstday = date_add('${EndDay}',interval -1 month) and ReportType = '月报' limit 1) b 
