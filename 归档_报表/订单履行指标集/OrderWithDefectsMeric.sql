...
select * 
	,round(OrderWithDefects_ord_cnt/monitor_ord_cnt,3)  as OrderDefectRate -- 有效追踪率
	,round(unnormal_shop_cnt/monitor_shop_cnt,3)  as unnormal_status_Shop_Rate  -- 状态异常店铺比例
from (
select
	count( distinct case when OrderDefectRateStatus in (2,3) then tmp.ShopCode  end) as unnormal_shop_cnt -- 最新情况
