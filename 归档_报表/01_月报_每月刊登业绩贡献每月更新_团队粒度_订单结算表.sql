...
select Id ,SellerSKU, ShopCode ,PublicationDate ,sku ,ProductSalesName  from import_data.erp_amazon_amazon_listing eaal 
union 
...
join import_data.erp_amazon_amazon_listing_delete eaald on eaal.SellerSKU = eaald.SellerSKU and eaal.ShopCode = eaald.ShopCode 
...
FROM
    ...
    WHERE
        ...
        GROUP BY
            `s`.`department`,`op`.`ShopIrobotId`,left(`PublicationDate`,7),left(`SettlementTime`,7),
...
WHERE
    ((`PublicationDate` > '2022-01-01 00:00:00')
        ...
        AND (`al`.`sku` != '')
    GROUP BY
        `s`.`department`, left(`PublicationDate`,7)
)
...
SELECT
    ...
    END AS `销售部门`,
    CASE
        WHEN `department` = '销售一部' THEN 'GM'
        WHEN `department` IN ('销售二部', '销售三部') THEN 'PM'
    END AS `模式`,
