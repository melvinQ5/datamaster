...
where wo.IsDeleted = 0 and PayTime >=date(date_add(CURRENT_DATE(),INTERVAL -31 day)) and PayTime<date(date_add(CURRENT_DATE(),INTERVAL -1 day))
and boxsku!='shopfee' and asin <>'' and boxsku <>'' and OrderStatus<>'作废'
group by ms.site,wo.asin,boxsku
order by sales desc
)

-- 1 UK站近30天出单数>3单的链接业绩
-- 2 所有站近30天出单数>3单的链接业绩
,t_odsku as( -- 按照sku聚合业绩数据
select site, round(sum(sales),2) sales,round(sum(profit),2)profit
    ,sum(orders)orders,round(sum(freightfee),2) freightfee,date(min(mintime)) mintime
    ,datediff(date_add(CURRENT_DATE(),INTERVAL -1 day),date(min(mintime)))saledays
    ,max(solddays)solddays,round(sum(orders)/max(solddays),2) `日均销量`
    ,round(sum(case when site='UK' then sales end),2) as uksale
from t_od
group by site
)

, skuprofit as ( -- 统计不含运费单（无运费收入的交易记录）用于计算boxsku的挂单利润率
select boxsku, round(sum((totalgross-feegross)/ExchangeUSD),2) sa,round(sum((totalprofit-feegross)/ExchangeUSD),2) pro
    ,round(sum(totalprofit)/sum(totalgross),4) nonfeerate  -- 剔除运费单后的利润率
    ,count(distinct platordernumber) nonfeeorders  -- 剔除运费单后的订单量
from wt_orderdetails wo
join import_data.mysql_store ms on ms.Code = wo.shopcode and ms.Department ='快百货'  and ms.NodePathName in ('快次方-成都销售组','快次元-成都销售组')
where wo.IsDeleted = 0 and PayTime >=date(date_add(CURRENT_DATE(),INTERVAL -31 day)) and PayTime<date(date_add(CURRENT_DATE(),INTERVAL -1 day))
...
,spuprofit as -- 计算SPU的挂单利润率、销售额、
    -- 按SPU算一遍剔除运费的  利润额、订单量
(
select spu
    , ROUND(sum(pro)/sum(sa),4) nonfeerate
    ,
from skuprofit
...
where createdtime>= date(date_add(CURRENT_DATE(),INTERVAL -31 day)) and createdtime<= date(date_add(CURRENT_DATE(),INTERVAL -1 day))
and al.sku<>''
...
where wo.IsDeleted = 0 and PayTime >=date(date_add(CURRENT_DATE(),INTERVAL -61 day)) and PayTime<date(date_add(CURRENT_DATE(),INTERVAL -31 day))
and boxsku!='shopfee' and asin <>'' and boxsku <>'' and OrderStatus<>'作废'
group by ms.site,wo.asin,boxsku
order by sales desc
)
...
left join mysql_store ms on ms.code=ads.shopcode and ms.NodePathName in ('快次方-成都销售组','快次元-成都销售组')
left join pp on pp.sku=al.sku
where createdtime>= date(date_add(CURRENT_DATE(),INTERVAL -61 day)) and createdtime<= date(date_add(CURRENT_DATE(),INTERVAL -31 day))
and al.sku<>''
group by  al.sku,pp.boxsku having pp.boxsku is not null
)
...
datediff(date_add(CURRENT_DATE(),INTERVAL -1 day),date(min(mintime)))saledays,max(solddays)lastsolddays,round(sum(orders)/max(solddays),2) `上个30天日均销量`,
...
where wl.IsDeleted = 0
	...
	and ms.NodePathName in ('快次方-成都销售组','快次元-成都销售组')
	and wl.ListingStatus =1 and ms.shopstatus = '正常' and wl.sku<>'' and wl.asin<>''
  and orders>=3 and orders<5
group by pp.boxsku
)b
left join(
select boxsku,count(distinct concat(asin,site)) lastalists from lastmontht_od
where orders>=3 and orders<5
group by boxsku) a on a.boxsku=b.boxsku
)
...
,spuresult as(
select '30天成都爆旺款SPU统计'mark,CURRENT_DATE() date,skuresult.spu,count(distinct case when skuresult.productstatus<>'停产'and orders>0 then  skuresult.boxsku end) `出单且正常SKU数`,row_number() over(order by sum(sales)desc ) as salesort,row_number() over(order by sum(orders)desc ) as ordersort,c.nonfeerate,d.maxorders,d.minorders,round(sum(sales),2)sales,round(sum(profit),2)profit,sum(orders)orders,round(sum(freightfee),2)freightfee,round(sum(`扣广告利润额(不含运费)`),2)`扣广告利润额(不含运费)`
