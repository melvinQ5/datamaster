...
, (case when sum((totalgross-FeeGross)/ExchangeUSD)>=1500 then '成都爆款' 
when  sum((totalgross-FeeGross)/ExchangeUSD)>=500 and  sum((totalgross-FeeGross)/ExchangeUSD)<1500 
then '成都旺款' when sum((totalgross-FeeGross)/ExchangeUSD)<500 then '成都出单' end) as producttype
...
-- 鑫晶 新品爆旺款清单
--  select * from cdtopspu where 新品 = '新品'
...
,listtype as (
select listdetail.*,(case when producttype in('成都爆款','成都旺款') and orders>=15 then 'S' when orders>=5 and orders<15 and producttype in('成都爆款','成都旺款') then 'A' when orders>=5 then 'B' when  orders>=1 and orders<5 then 'C' end) as listtype
from listdetail
...
,listtype0 as (
select listdetail0.*,(case when producttype0 in('成都爆款','成都旺款') and orders>=15 then 'S' when orders>=5 and orders<15 and producttype0 in('成都爆款','成都旺款') then 'A' when orders>=5 then 'B' when  orders>=1 and orders<5 then 'C' end) as listtype0
from listdetail0
-- where producttype is not null
)
...
left join cdtopspu0 on cdtopspu0.spu=cdtopspu.spu and  producttype0 in ('成都爆款','成都旺款') 
where producttype0 is null 
and producttype in ('成都爆款','成都旺款')

)

,deleteallproduct as( -- 统计2个周次中爆旺款减少
select '新品爆旺款减少'mark,cdtopspu0.* from cdtopspu0
left join cdtopspu on cdtopspu.spu=cdtopspu0.spu and  producttype in ('成都爆款','成都旺款') 
where producttype is null 
and producttype0 in ('成都爆款','成都旺款')
)
...
,addproduct as (
select '新品爆旺款新增'mark, cdtopspu.*from cdtopspu
left join cdtopspu0 on cdtopspu0.spu=cdtopspu.spu and  producttype0 in ('成都爆款','成都旺款') and cdtopspu0.新品 is not null
where producttype0 is null 
and producttype in ('成都爆款','成都旺款')
...
select '新品爆旺款减少'mark,cdtopspu0.* from cdtopspu0
left join cdtopspu on cdtopspu.spu=cdtopspu0.spu and  producttype in ('成都爆款','成都旺款') and cdtopspu.新品 is not null
where producttype is null 
