 ...
 */
/*
 每周四，通过成都30天版分析爆旺款涨跌原因，通过成都14天版标注高潜力商品
 成都30天 、 成都14天
 */
...
,add_index_3 as (
select spu ,group_concat( UK超3单业绩占比 ) UK超3单业绩占比 ,group_concat(DE超3单业绩占比) DE超3单业绩占比 ,group_concat(FR超3单业绩占比) FR超3单业绩占比
...
,add_index_5 as (
select ol.spu ,count(distinct ol.sku) `出单且正常的SKU数`
from od_list_in30d ol join wt_products wp on ol.sku = wp.sku and wp.IsDeleted=0 and wp.ProductStatus != 2
group by ol.spu
)

,add_attr_1 as (
select pm.spu ,epp.productname `商品名称`  ,epps. StartNumber `起批量` ,eppila.LogisticAttributeName `物流属性`
from prod_mark pm
