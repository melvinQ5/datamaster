...
select wo.asin,ms.site,boxsku, round(sum((totalgross-feegross)/ExchangeUSD),2) sales,round(sum((totalprofit-feegross)/ExchangeUSD),2) profit,count(distinct platordernumber) orders,round(sum(feegross/ExchangeUSD),2) freightfee,round(sum(-RefundAmount),2)refund,date(min(paytime)) mintime, datediff(date_add(CURRENT_DATE(),INTERVAL -1 day),date(min(paytime)))saledays,count(distinct date(PayTime))solddays,round(sum((totalgross-feegross)/ExchangeUSD)/( datediff(date_add(CURRENT_DATE(),INTERVAL -1 day),date(min(paytime)))),2) `日均销量`,row_number() over(order by count(distinct platordernumber) desc ) as ordersort,row_number() over(order by  round(sum((totalgross-feegross)/ExchangeUSD),2)  desc ) as salessort

from wt_orderdetails wo
join import_data.mysql_store ms on ms.Code = wo.shopcode and ms.Department ='快百货'  and ms.NodePathName in ('快次方-成都销售组','快次元-成都销售组')

where wo.IsDeleted = 0 and PayTime >=date(date_add(CURRENT_DATE(),INTERVAL -15 day)) and PayTime<date(date_add(CURRENT_DATE(),INTERVAL -1 day))
and boxsku!='shopfee' and asin <>'' and boxsku <>'' and OrderStatus<>'作废'
...
select boxsku, round(sum(sales),2) sales,round(sum(profit),2)profit,sum(orders)orders,round(sum(freightfee),2) freightfee,date(min(mintime)) mintime,datediff(date_add(CURRENT_DATE(),INTERVAL -1 day),date(min(mintime)))saledays,max(solddays)solddays,round(sum(orders)/max(solddays),2) `日均销量`,round(sum(case when site='UK' then sales end),2) as uksale,round(sum(case when site='DE' then sales end),2) as desale,round(sum(case when site='FR' then sales end),2) as frsale,round(sum(case when site='US' then sales end),2) as ussale
from t_od
group by boxsku
)

, skuprofit as ( -- 统计不含运费单（无运费收入的交易记录）用于计算boxsku的挂单利润率
select boxsku, round(sum((totalgross-feegross)/ExchangeUSD),2) sa,round(sum((totalprofit-feegross)/ExchangeUSD),2) pro ,round(sum(totalprofit)/sum(totalgross),4) nonfeerate
from wt_orderdetails wo
join import_data.mysql_store ms on ms.Code = wo.shopcode and ms.Department ='快百货'  and ms.NodePathName in ('快次方-成都销售组','快次元-成都销售组')
where wo.IsDeleted = 0 and PayTime >=date(date_add(CURRENT_DATE(),INTERVAL -15 day)) and PayTime<date(date_add(CURRENT_DATE(),INTERVAL -1 day))
and boxsku!='shopfee' and asin <>'' and boxsku <>''and OrderStatus<>'作废' and TransactionType='付款' and FeeGross=0
group by boxsku
order by sa desc
)

,spuprofit as -- 计算SPU的挂单利润率
(
select spu, ROUND(sum(pro)/sum(sa),4) nonfeerate
from skuprofit
join pp on pp.boxsku=skuprofit.boxsku
group by spu
)


, t_oddate as ( -- 统计每日boxsku订单量，用于计算每日最大单量）
select date(PayTime) date,wo.boxsku,pp.spu,count(distinct platordernumber) orders
from wt_orderdetails wo
join pp on pp.boxsku=wo.boxsku
join import_data.mysql_store ms on ms.Code = wo.shopcode and ms.Department ='快百货'  and ms.NodePathName in ('快次方-成都销售组','快次元-成都销售组')
where wo.IsDeleted = 0 and PayTime >=date(date_add(CURRENT_DATE(),INTERVAL -15 day)) and PayTime<date(date_add(CURRENT_DATE(),INTERVAL -1 day))
and wo.boxsku!='shopfee' and asin <>'' and wo.boxsku <>'' and OrderStatus<>'作废'
group by date(PayTime),wo.boxsku,pp.spu
)

,skuorder as  -- 统计boxsku每日最大单量
(
select boxsku,max(orders)maxorders,min(orders)minorders from t_oddate
group by boxsku
)

,spuorder as( -- 统计spu每日最大单量
select spu,max(orders)maxorders,min(orders)minorders from
(select date,spu,sum(orders)orders from t_oddate
group by date,spu
)a
group by spu
)
...
left join mysql_store ms on ms.code=ads.shopcode and  ms.NodePathName in ('快次方-成都销售组','快次元-成都销售组')
left join pp on pp.sku=al.sku
where createdtime>= date(date_add(CURRENT_DATE(),INTERVAL -15 day)) and createdtime<= date(date_add(CURRENT_DATE(),INTERVAL -1 day))
and al.sku<>''
...
, sitecal as ( -- 近30天数据聚合
select t_odsku.boxsku boxsku1,pp.SKU ,SPU SPU1,ProjectTeam,AuditTime,productname,ele_name,torttype_name,productstatus,row_number() over(order by sum(sales)desc ) as salesort,row_number() over(order by sum(orders)desc ) as ordersort,round(sum(sales),2) sales,round(sum(profit),2)profit
...
select wo.asin,ms.site,boxsku, round(sum((totalgross-feegross)/ExchangeUSD),2) sales,round(sum((totalprofit-feegross)/ExchangeUSD),2) profit,round( sum((totalprofit-feegross))/sum((totalgross-feegross)),4) profitrate,count(distinct platordernumber) orders,round(sum(feegross/ExchangeUSD),2) freightfee,round(sum(-RefundAmount),2) refund,date(min(paytime)) mintime, datediff(date_add(CURRENT_DATE(),INTERVAL -1 day),date(min(paytime)))saledays,count(distinct date(PayTime))solddays,round(sum((totalgross-feegross)/ExchangeUSD)/( datediff(date_add(CURRENT_DATE(),INTERVAL -1 day),date(min(paytime)))),2) `日均单数`,row_number() over(order by count(distinct platordernumber) desc ) as ordersort,row_number() over(order by  round(sum((totalgross-feegross)/ExchangeUSD),2)  desc ) as salessort
from wt_orderdetails wo
join import_data.mysql_store ms on ms.Code = wo.shopcode and ms.Department ='快百货'  and  ms.NodePathName in ('快次方-成都销售组','快次元-成都销售组')

where wo.IsDeleted = 0 and PayTime >=date(date_add(CURRENT_DATE(),INTERVAL -29 day)) and PayTime<date(date_add(CURRENT_DATE(),INTERVAL -15 day))
and boxsku!='shopfee' and asin <>'' and boxsku <>'' and OrderStatus<>'作废'
group by ms.site,wo.asin,boxsku
order by sales desc
)
...
left join mysql_store ms on ms.code=ads.shopcode and ms.NodePathName in ('快次方-成都销售组','快次元-成都销售组')
left join pp on pp.sku=al.sku
where createdtime>= date(date_add(CURRENT_DATE(),INTERVAL -29 day)) and createdtime<= date(date_add(CURRENT_DATE(),INTERVAL -15 day))
and al.sku<>''
group by  al.sku,pp.boxsku having pp.boxsku is not null
)
...
datediff(date_add(CURRENT_DATE(),INTERVAL -1 day),date(min(mintime)))saledays,max(solddays)lastsolddays,round(sum(orders)/max(solddays),2) `上个14天日均销量`,
...
,onlinelist as(
select pp.BoxSku ,count(distinct concat(asin,site)) listings,count(distinct ms.companycode) shops
...
--
,sku as(
select distinct a.boxsku,spu,ProjectTeam,
AuditTime,
productname,
ele_name,
torttype_name,
...
,skuresult as( -- 按照sku分析成都的爆旺款
select'14天成都爆旺款SKU统计'mark,CURRENT_DATE() date, sku.boxsku,sku.spu,sku.ProjectTeam,sku.AuditTime,sku.productname,sku.ele_name,sku.torttype_name,sku.productstatus,salesort,ordersort,sales,profit,orders,freightfee,销售天数,日均销量,exposure,clicks,spend,adorders,adsales,ctr,cvr,cpc,acost,ROI,uksale,desale,frsale,ussale,
nonfeerate,h.maxorders,h.minorders,
if(spend is null,round(profit/sales,4),round((profit-spend)/sales,4) )profitrate,if(spend is null,profit,round(profit-spend,2))`扣广告利润额(不含运费)`,if(spend is null,round(profit+freightfee,2),round((profit-spend+freightfee),2))`扣广告含运费利润额`,round(uksale/sales,4) uksalerate,round(desale/sales,4)desalerate,round(frsale/sales,4)frsalerate,round(ussale/sales,4)ussalerate
,shops,listings,Slist,alist,c.SellUserName,c.NodePathName
,lastSlists,lastalists,lastsalesort,lastordersort,lastsales,lastprofit
...
,`上个14天日均销量`,lastuksale,lastdesale,lastfrsale,lastussale,lastexposure,lastclicks,lastspend,lastadorders,lastadsales,lastctr,lastcvr,lastcpc,lastacost,lastROI
from sku
...
left join skuprofit g on g.boxsku=sku.boxsku
left join skuorder h on h.boxsku=sku.boxsku
order by sales desc,salesort asc
)
...
,spuresult as(
select '14天成都爆旺款SPU统计'mark,CURRENT_DATE() date,skuresult.spu,count(distinct case when skuresult.productstatus<>'停产'and orders>0 then  skuresult.boxsku end) `出单且正常SKU数`,row_number() over(order by sum(sales)desc ) as salesort,row_number() over(order by sum(orders)desc ) as ordersort,c.nonfeerate,d.maxorders,d.minorders,round(sum(sales),2)sales,round(sum(profit),2)profit,sum(orders)orders,round(sum(freightfee),2)freightfee,round(sum(`扣广告利润额(不含运费)`),2)`扣广告利润额(不含运费)`
...
round(sum(uksale),2)uksale,round(sum(desale),2)desale,round(sum(frsale),2)frsale,round(sum(ussale),2)ussale,round(sum(uksale)/sum(sales),2)uksalerate,round(sum(desale)/sum(sales),2)desalerate,round(sum(frsale)/sum(sales),2)frsalerate,round(sum(ussale)/sum(sales),2)ussalerate,max(shops)shops,
sum(listings)listings,sum(Slist)Slist,sum(alist)alist,GROUP_CONCAT(SellUserName)SellUserName,GROUP_CONCAT(NodePathName)NodePathName,sum(lastSlists)lastSlists,sum(lastalists)lastalists,row_number() over(order by sum(lastsales)desc ) as lastsalesort,row_number() over(order by sum(lastorders)desc ) as lastordersort,round(sum(lastsales),2) lastsales,round(sum(lastprofit),2)lastprofit,sum(lastorders)lastorders,round(sum(lastfreightfee),2) lastfreightfee,max(lastsolddays)`上个14天销售天数`,round(sum(lastorders)/max(lastsolddays),2)`上个14天日均单量`,round(sum(lastuksale),2) as lastuksale,round(sum(lastdesale),2) as lastdesale,round(sum(lastfrsale),2) as lastfrsale,round(sum(lastussale),2) as lastussale,sum(lastexposure)lastexposure,sum(lastclicks)lastclicks,sum(lastspend) lastspend,sum(lastadorders) lastadorders,sum(lastadsales) lastadsales,round(sum(lastclicks)/sum(lastexposure),4) lastctr,round(sum(lastadorders)/sum(lastclicks),4) lastcvr,round(sum(lastspend)/sum(lastclicks),4) lastcpc, round(sum(lastSPEND)/sum(lastadsales),4) lastacost, round(sum(lastadsales)/sum(lastspend),2) lastROI

from skuresult
left join spuprofit c on c.spu=skuresult.spu
left join spuorder d on d.spu=skuresult.spu
where skuresult.spu<>''
group by skuresult.spu,c.nonfeerate,d.maxorders,d.minorders
