...
from (
    ...
    where  isdeleted=0 and  FirstDay >= date_add( subdate(' ${NextStartDay}', date_format(' ${NextStartDay}', '%w')-1), interval - 5 week )
...
select OrderNumber ,PlatOrderNumber ,TotalGross,TotalProfit,TotalExpend ,shopcode ,asin
    ...
    ,case when vknp.sku is not null
...
select
	...
	,round(TotalProfit_weekly - ifnull(ad_Spend,0),2) `当周扣广告利润额`
	,ifnull(orders_weekly,0) `当周订单量`
	,ifnull(salecount_weekly,0) `当周sku销量`
...
from 
	...
	join ( select distinct week_num_in_year,week_begin_date from dim_date
		where full_date >= date_add(  subdate('${NextStartDay}',date_format('${NextStartDay}','%w')-1)  , INTERVAL -7*2 DAY) and full_date < subdate('${NextStartDay}',date_format('${NextStartDay}','%w')-1)
		) dd
