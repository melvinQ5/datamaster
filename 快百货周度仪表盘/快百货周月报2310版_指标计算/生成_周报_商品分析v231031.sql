...
insert into manual_table_duplicate (wttime,c1,c2,c3,c4,c5,c6,c7,c8,c9,c10,c11,c12,c13,c14,c15,c16,c17,c18,c19,c20,c21,c22,c23,c24,c25,c26,c27,c28,c29,c30,c31)
with
prod as (
select wp.sku ,wp.spu ,wp.BoxSku
  ...
  ,left(DevelopLastAuditTime,7) 终审年月
    ,wp.cat1 一级类目
  ,case when wp.DevelopLastAuditTime >=  date_add( DATE_ADD('${StartDay}',interval -day( '${StartDay}'  )+1 day) ,interval -'${NewProdMonths}' month) -- 测算次月目标时 NewProdMonths = 1
...
,res2 as ( -- 产品聚合
select now() wttime , '${ReportType}' as reporttype ,自然周月 ,'${StartDay}' as firstday
    ,新老品
     ,case when 优先级元素 regexp '圣诞节|冬季' then '主题品' else '非主题品' end as  主题 ,优先级元素 ,高潜,一级类目  ,链接刊登划分  ,区域
    ,round(sum(订单量),0) 订单量
    -- ,round(sum(销量),0) 销量
    ...
    ,count( distinct concat(online_sellersku,shopcode)) 在线链接数
from res1
group by 新老品   ,优先级元素 ,高潜,一级类目  ,链接刊登划分 ,区域 ,自然周月 ,当期第一天 ,是否结算项记录
)

,res3 as ( -- 产品聚合
select wttime ,reporttype ,自然周月 ,firstday ,新老品,主题 ,优先级元素,高潜,一级类目  ,链接刊登划分  ,区域
    ,订单量  ,出单SPU数, 销售额,利润额_扣ad , 销售额_扣运费未扣退款 , 利润额_扣运费未扣退款,  退款额 , 运费收入 ,交易成本
    ,采购成本 , 物流成本, 广告曝光量, 广告花费 , 广告销售额 , 广告销量 , 广告点击量 ,总未停产SPU数 ,总在仓产品金额_万元 , 是否结算项记录 ,在线渠道SKU数 , 在线链接数
from res2 a,prod_stat,ware_stat
)
