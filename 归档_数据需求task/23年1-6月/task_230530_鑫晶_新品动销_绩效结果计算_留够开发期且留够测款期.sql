...
where date_add(epp.DevelopLastAuditTime,interval - 8 hour) >=  date_add('${StartDay}',interval - 14 day ) and date_add(epp.DevelopLastAuditTime,interval - 8 hour) < date_add('${NextStartDay}',interval - 14 day )
...
select * from (
	...
	from (
		...
		join import_data.mysql_store ms on ms.Code = od.shopcode and od.IsDeleted = 0
			and ms.Department ='快百货' and PayTime >= date_add('${StartDay}',interval - 14 day )
...
select
    concat(date(date_add('${StartDay}',interval - 14 day )) ,'至', date(date_add('${NextStartDay}',interval - 14-1 day )))  '终审时间范围'
     ,*
from (
	select
