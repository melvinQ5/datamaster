...
and pp.DevelopLastAuditTime<'2023-08-01'
and (pp.ProductStopTime >='2023-04-01' or pp.ProductStopTime is null)
)
...
select  ASIN, site,sku,PublicationDate,TIMESTAMPDIFF(month,(case when PublicationDate>='2023-04-01'then PublicationDate else'2023-04-01' end),'2023-07-31')+1 链接可售月份 from (
...
union
select  ASIN, site,sku,PublicationDate,TIMESTAMPDIFF(month,(case when PublicationDate>='2023-04-01'then PublicationDate else'2023-04-01' end),LastModificationTime)+1 链接可售月份 from (
...
join mysql_store s on s.code =al.shopcode and s.Department='快百货' and s.nodepathname regexp '${team1}|${team2}'
and LastModificationTime>='2023-04-01' and LastModificationTime<'2023-08-01'
where al.sku <>''
)b where sort=1
)c 
)d where sort=1
...
where Year='2023' and week>=15 and week<=31
)a 
...
and department='快百货'
and paytime>='2023-04-01' and paytime<'2023-08-01' 
and orderstatus<>'作废'
...
平均链接销售月份,
链接可售月份
from (select spu,min(AuditTime)AuditTime, DATE_FORMAT(min(AuditTime),'%Y-%m-01') developmon,(case when min(AuditTime)<'2023-04-01' then 4 else (TIMESTAMPDIFF(month,DATE_FORMAT(min(AuditTime),'%Y-%m-01'),'2023-07-01')+1) end) 商品可售月份 from epp 
-- where ele_name='其他' 
group by spu )a
