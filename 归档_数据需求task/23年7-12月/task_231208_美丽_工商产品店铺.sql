...
,pre_st as ( -- 给定店铺
select  sku,SellUserName ,在线账号代码 ,c1
from (
    select sku,SellUserName ,在线账号代码 ,split(在线账号代码,',')[1] c1 from res
    union select sku,SellUserName ,在线账号代码 ,split(在线账号代码,',')[2] c2 from res
    union select sku,SellUserName ,在线账号代码 ,split(在线账号代码,',')[3] c3 from res
    union select sku,SellUserName ,在线账号代码 ,split(在线账号代码,',')[4] c4 from res
    union select sku,SellUserName ,在线账号代码 ,split(在线账号代码,',')[5] c5 from res
    ) t
where c1 is not null
order by  sku,SellUserName )

, fliter as (
select distinct  sku ,SellUserName  from (
select * ,case when c1 regexp 'QF|NN|NX|EI|ZK|NA|YC|ZR|ZE|ZA|ZB|YX|YP|XY|B208' then 1 else 0 end isbelong
from pre_st ) t
where isbelong = 1  )


select res.* ,ifnull(c3,'无新类目') 一级类目,ifnull(c4,'无新类目') 二级类目 ,ifnull(c5,'无新类目') 三级类目
from res join fliter on res.sku =fliter.sku and res.SellUserName= fliter.SellUserName
left join manual_table j on  handlename='美丽_快百货新旧类目对照_231208' and j.memo=res.sku
;
