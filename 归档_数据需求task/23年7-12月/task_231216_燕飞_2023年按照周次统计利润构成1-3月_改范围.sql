...
,split_part(pc.CategoryPathByChineseName,'>',1) category1 , CONCAT(split_part(pc.CategoryPathByChineseName,'>',1) ,'>',split_part(pc.CategoryPathByChineseName,'>',2) ) category2 ,CONCAT(split_part(pc.CategoryPathByChineseName,'>',1) ,'>',split_part(pc.CategoryPathByChineseName,'>',2),'>',split_part(pc.CategoryPathByChineseName,'>',3) ) category3 ,CONCAT(split_part(pc.CategoryPathByChineseName,'>',1) ,'>',split_part(pc.CategoryPathByChineseName,'>',2),'>',split_part(pc.CategoryPathByChineseName,'>',3),'>',split_part(pc.CategoryPathByChineseName,'>',4) ) category4,CONCAT(split_part(pc.CategoryPathByChineseName,'>',1) ,'>',split_part(pc.CategoryPathByChineseName,'>',2),'>',split_part(pc.CategoryPathByChineseName,'>',3),'>',split_part(pc.CategoryPathByChineseName,'>',4),'>',split_part(pc.CategoryPathByChineseName,'>',5) ) category5,IFNULL(fes.ele_name_priority,'其他')ele_name,case pp.productstatus
when 0 then '正常'
when 2 then '停产'
when 3 then '停售'
when 4 then '暂时缺货'
...
join ( -- 指定四个优先级元素
    select distinct sku ,ele_name_priority  from dep_kbh_product_test where ele_name_priority regexp '园艺|夏季|万圣|圣诞'
...
and pp.DevelopLastAuditTime<'2023-04-01'
...
select  ASIN, site,sku,PublicationDate,TIMESTAMPDIFF(month,(case when PublicationDate>='2023-01-01'then PublicationDate else'2023-01-01' end),'2023-04-30')+1 链接可售月份 from (
...
join mysql_store s on s.code =al.shopcode and s.Department='快百货' and s.nodepathname regexp '${team1}|${team2}'
and LastModificationTime>='2023-01-01' and LastModificationTime<'2023-04-01'
where al.sku <>''
)b where sort=1
)c 
)d where sort=1
...
,ad as(
select a.sku1 sku, spu,year,week,asin,site,sum(AdExposure)AdExposure,sum(adclicks)adclicks,round(sum(adspend),2)adspend,round(sum(adsales),2)adsales ,round(sum(adsaleunits),2)adorders
from (
...
where Year='2023' and week>=2 and week<=14
) a
...
and department='快百货'
and paytime>='2023-01-01' and paytime<'2023-04-01'
and orderstatus<>'作废'
...
平均链接销售月份,
链接可售月份
from (select spu,min(AuditTime)AuditTime, DATE_FORMAT(min(AuditTime),'%Y-%m-01') developmon,(case when min(AuditTime)<'2023-01-01' then 3 else (TIMESTAMPDIFF(month,DATE_FORMAT(min(AuditTime),'%Y-%m-01'),'2023-03-01')+1) end) 商品可售月份 from epp 
-- where ele_name='其他' 
group by spu )a
