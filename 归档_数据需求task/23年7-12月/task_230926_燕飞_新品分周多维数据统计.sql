...
with
tb as ( -- 燕飞给定一份产品多元素优先级判定
select c1+0 as sort ,c2 as arr
from manual_table where handlename='产品主元素优先级231016'
)
...
from (
    select sku
        ,sort
        ,case when tb.arr is null then '不在优先级名单' else pre_ele_name end as ele_name
    from pre_ele a
...
,prod as (
select vknp.sku
  ,case when ele.ele_name is null then '无元素标签' else ele.ele_name end ele_name
...
from wt_adserving_amazon_weekly  asa -- 保留所有打标签链接，并对有曝光数据的链接进行行拆分
join ( select distinct wl.id ,wl.sku ,ele_name ,dev_month from wt_listing wl
	...
	and asa.Year = 2023 and asa.week >= 27 -- 7月3日是按dim_date的28周，对应广告周表的27周
)
...
join (select distinct code as shopcode ,CompanyCode , AccountCode
    , case when NodePathName regexp  '成都' then '快百货一部' else '快百货二部' end as 区域
