...
and (pp.ProductStopTime >='2023-08-01' or pp.ProductStopTime is null)
and AuditTime >= '2023-03-10' and  AuditTime < '2023-11-01 '-- 统计3月10日之后的SPU 
)
...
select  ASIN, site,sku,PublicationDate,TIMESTAMPDIFF(month,(case when PublicationDate>='2023-08-01'then PublicationDate else'2023-08-01' end),'2023-11-30')+1 链接可售月份 from (
...
union
select  ASIN, site,sku,PublicationDate,TIMESTAMPDIFF(month,(case when PublicationDate>='2023-08-01'then PublicationDate else'2023-08-01' end),LastModificationTime)+1 链接可售月份 from (
...
join mysql_store s on s.code =al.shopcode and s.Department='快百货' and s.nodepathname regexp '${team1}|${team2}'
and LastModificationTime>='2023-08-01' and LastModificationTime<'2023-11-01'
where al.sku <>''
)b where sort=1
)c 
)d where sort=1
...
where Year='2023' and week>=32 and week<=45 
) a 
...
and department='快百货'
and paytime>='2023-08-01' and paytime<'2023-11-01' 
and orderstatus<>'作废'
...
,spuhuizong as(
select a.spu,a.developmon,ifnull(a.商品可售月份,0)商品可售月份,(case when AuditTime>='2023-03-10' then '新品' else '老品' end) prod,主题,
orders,
销量,
sales,
profit,
...
链接可售月份,
AuditTime
from (select spu,min(AuditTime)AuditTime, DATE_FORMAT(min(AuditTime),'%Y-%m-01') developmon,(case when min(AuditTime)<'2023-08-01' then 8 else (TIMESTAMPDIFF(month,DATE_FORMAT(min(AuditTime),'%Y-%m-01'),'2023-11-01')+1) end) 商品可售月份 from epp 
group by spu )a
...
, res as (
select spu  ,'8-10月' 统计时间, '新品' prod ,AuditTime 终审时间
,round(sum(case when orders>0 then sales end),2) sales
,round(sum(case when orders>0 then profitnew end),2) profitnew
-- ,count(distinct spu) spu总数
-- ,count(case when lists>0 then spu end)刊登SPU数
,sum(case when lists>0 then lists end)刊登总条数
-- ,count(case when orders>0 then spu end) 出单SPU数
-- ,round(count(case when orders>0 then spu end) /count(distinct spu),6)spu动销率
-- ,round(avg(case when orders>0 then orders end),3) 出单平均订单数
-- ,round(avg(case when orders>0 then 销量 end),3) 出单平均销量
,sum(曝光链接数)曝光链接数
,sum(AdExposure)AdExposure
