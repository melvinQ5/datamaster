...
, lw_pre as ( -- local_warehouse 上期本地仓表
select 
	case when tmp.BoxSKU is null then 0 else 1 end as is_order -- 0=未出单，1=有出单
	, TotalPrice, TotalInventory, pt.BoxSKU, pt.category, pt.id as ProductId, pt.is_new, pt.is_lead, pt.is_other
FROM import_data.WarehouseInventory wi
...
left join -- 用于计算是否本周出单
	(select distinct BoxSku from od) tmp on wi.BoxSku = tmp.BoxSKU 
where WarehouseName = '东莞仓' and Monday < date_add('${EndDay}',interval -7 day) and Monday >= date_add('${EndDay}',interval -14 day) and ReportType = '周报'
)
...
select po.* 
	...
	, pr.PurchaseOrderNo as is_arrived -- 已到货
...
left join import_data.PurchaseRev pr on pr.PurchaseOrderNo = po.PurchaseOrderNo -- 用于计算N天到货率
)

, pd as ( -- PackageDetail包裹表 关联物流轨迹表
...
select 
	...
	, case when scantime is not null and timestampdiff(second, ordertime, scantime) < 86400 * 5 then pr.PurchaseOrderNo -- 有扫描时间，且扫描时间 - 订单时间小于5天 的采购订单
	when scantime is null and ((instockquantity > 0) or ( CompleteTime is not null and IsComplete = '否' )) 
	and timestampdiff(second, ordertime, CompleteTIme) < 86400 * 5 then pr.PurchaseOrderNo -- 没有扫描时间，入库数量大于零, 且入库时间 - 订单时间小于5天 的采购订单
	end as in5days_rev_numb -- 满足5天到货的采购单号
from import_data.PurchaseOrder po
left join import_data.PurchaseRev pr on po.PurchaseOrderNo = pr.PurchaseOrderNo -- 用于计算N天到货率
...
select '所有类目' as category,'所有部门' as department, '周报' as ReportType, weekofyear('${EndDay}') as static_date,'所有产品' as product_tupe
	, round(count(distinct in5days_rev_numb)/count(distinct PurchaseOrderNo),4) `采购5天到货率` 
from po_pre
union all -- 分类目
select category,'所有部门' as department, '周报' as ReportType, weekofyear('${EndDay}') as static_date,'所有产品' as product_tupe
	, round(count(distinct in5days_rev_numb)/count(distinct PurchaseOrderNo),4) `采购5天到货率` 
...
select '所有类目' as category,'所有部门' as department, '周报' as ReportType, weekofyear('${EndDay}') as static_date,'所有产品' as product_tupe
	, sum(Price - DiscountedPrice) `采购产品金额` from po where !(IsComplete = '是' and InstockQuantity = 0)
union all -- 分类目
select category,'所有部门' as department, '周报' as ReportType, weekofyear('${EndDay}') as static_date,'所有产品' as product_tupe
	, sum(Price - DiscountedPrice) `采购产品金额` from po left join pt on pt.BoxSKU = po.BoxSKU where !(IsComplete = '是' and InstockQuantity = 0) group by category 
union all -- 分品类（新品）
select '所有类目' as category,'所有部门' as department, '周报' as ReportType, weekofyear('${EndDay}') as static_date,'新品' as product_tupe
	, sum(Price - DiscountedPrice) `采购产品金额` from po left join pt on pt.BoxSKU = po.BoxSKU where is_new=1 and !(IsComplete = '是' and InstockQuantity = 0)
union all -- 分品类（重点）
select '所有类目' as category,'所有部门' as department, '周报' as ReportType, weekofyear('${EndDay}') as static_date,'重点' as product_tupe
	, sum(Price - DiscountedPrice) `采购产品金额` from po left join pt on pt.BoxSKU = po.BoxSKU where is_lead=1 and !(IsComplete = '是' and InstockQuantity = 0)
union all -- 分品类（其他）
select '所有类目' as category,'所有部门' as department, '周报' as ReportType, weekofyear('${EndDay}') as static_date,'其他' as product_tupe
	, sum(Price - DiscountedPrice) `采购产品金额` from po left join pt on pt.BoxSKU = po.BoxSKU where is_other=1 and !(IsComplete = '是' and InstockQuantity = 0)
union all -- 分品类（新品）+分类目
select category,'所有部门' as department, '周报' as ReportType, weekofyear('${EndDay}') as static_date,'新品' as product_tupe
	, sum(Price - DiscountedPrice) `采购产品金额` from po left join pt on pt.BoxSKU = po.BoxSKU where is_new=1 and !(IsComplete = '是' and InstockQuantity = 0)  group by category
union all -- 分品类（重点）+分类目
select category,'所有部门' as department, '周报' as ReportType, weekofyear('${EndDay}') as static_date,'重点' as product_tupe
	, sum(Price - DiscountedPrice) `采购产品金额` from po left join pt on pt.BoxSKU = po.BoxSKU where is_lead=1 and !(IsComplete = '是' and InstockQuantity = 0) group by category 
union all -- 分品类（其他）+分类目
select category,'所有部门' as department, '周报' as ReportType, weekofyear('${EndDay}') as static_date,'其他' as product_tupe
	, sum(Price - DiscountedPrice) `采购产品金额` from po left join pt on pt.BoxSKU = po.BoxSKU where is_other=1 and !(IsComplete = '是' and InstockQuantity = 0) group by category 
)
...
from (select distinct(PurchaseOrderNo), Freight fr from po where !(IsComplete = '是' and InstockQuantity = 0)) a 
union all -- 分类目
...
from ( select (Price - DiscountedPrice)/(PayPrice-Freight)*Freight as fr , category from po left join pt on pt.BoxSKU = po.BoxSKU where !(IsComplete = '是' and InstockQuantity = 0) ) tmp 
group by category 
union all -- 分品类(新品)
...
from ( select (Price - DiscountedPrice)/(PayPrice-Freight)*Freight as fr from po left join pt on pt.BoxSKU = po.BoxSKU where is_new=1 and !(IsComplete = '是' and InstockQuantity = 0) ) tmp 
union all -- 分品类(重点)
...
from ( select (Price - DiscountedPrice)/(PayPrice-Freight)*Freight as fr from po left join pt on pt.BoxSKU = po.BoxSKU where is_lead=1 and !(IsComplete = '是' and InstockQuantity = 0) ) tmp 
union all -- 分品类(其他)
...
from ( select (Price - DiscountedPrice)/(PayPrice-Freight)*Freight as fr from po left join pt on pt.BoxSKU = po.BoxSKU where is_other=1 and !(IsComplete = '是' and InstockQuantity = 0) ) tmp 
union all -- 分品类(新品)+分类目
...
from ( select (Price - DiscountedPrice)/(PayPrice-Freight)*Freight as fr , category from po left join pt on pt.BoxSKU = po.BoxSKU where is_new=1 and !(IsComplete = '是' and InstockQuantity = 0) ) tmp 
group by category 
union all -- 分品类(重点)+分类目
...
from ( select (Price - DiscountedPrice)/(PayPrice-Freight)*Freight as fr , category from po left join pt on pt.BoxSKU = po.BoxSKU where is_lead=1 and !(IsComplete = '是' and InstockQuantity = 0) ) tmp 
group by category 
union all -- 分品类(其他)+分类目
...
from ( select (Price - DiscountedPrice)/(PayPrice-Freight)*Freight as fr , category from po left join pt on pt.BoxSKU = po.BoxSKU where is_other=1 and !(IsComplete = '是' and InstockQuantity = 0) ) tmp 
group by category 
)
-- 
-- 
...
union all -- 分类目
select category,'所有部门' as department, '周报' as ReportType, weekofyear('${EndDay}') as static_date,'所有产品' as product_tupe, round(sum(pc)) `发货订单采购金额usd` 
...
, local_w_pre as (  -- 在仓产品金额，sku库存件数，sku数 ，这里的 static_date 不能变,以便后续left join是对齐后一周期
SELECT '所有类目' as category,'所有部门' as department, '周报' as ReportType, weekofyear('${EndDay}') as static_date,'所有产品' as product_tupe
	, sum(TotalPrice) `上期在仓产品金额`  FROM lw_pre where TotalInventory > 0
union all -- 分类目
SELECT category,'所有部门' as department, '周报' as ReportType, weekofyear('${EndDay}') as static_date,'所有产品' as product_tupe
	, sum(TotalPrice) `上期在仓产品金额`  FROM lw_pre where TotalInventory > 0 group by category
union all -- 分品类
SELECT '所有类目' as category,'所有部门' as department, '周报' as ReportType, weekofyear('${EndDay}') as static_date, product_tupe
	, sum(TotalPrice) `上期在仓产品金额` 
	from (select *, '新品' as product_tupe from lw_pre where is_new=1 
		union all select *, '重点' as product_tupe from lw_pre where is_lead=1 
		union all select *, '其他' as product_tupe from lw_pre where is_other=1 ) tmp
	where TotalInventory > 0
	group by product_tupe
union all -- 分品类+分类目
SELECT category,'所有部门' as department, '周报' as ReportType, weekofyear('${EndDay}') as static_date, product_tupe
...
-- 	, spt.`月度SKU采购频次`
	, gadp.`2天生包率` , gadp.`订单5天发货率`, gadp.`订单7天发货率`, d2r.`24小时发货率`, r4r.`48小时收货率`
	, purc.`采购单数`, purc.`零散采购单数`, purc.`供应商数量`, purc.`供应商数（供货>1000元)`, purc.`对应供货金额`, purc.`总供货金额`, pa5r.`采购5天到货率`
	, lw.`在仓产品金额`, lwp.`上期在仓产品金额`, lw.`在仓sku件数`, lw.`在仓sku数`, pp.`采购产品金额`, pf.`采购运费`, dpa.`发货订单采购金额usd`
...
select category, department, ReportType, static_date, product_tupe
	, `订单5天发货率`, `订单7天发货率`, `2天生包率`, `采购5天到货率`, `48小时收货率`, `24小时发货率`, `采购单数`, `零散采购单数`, `供应商数量`, `供应商数（供货>1000元)`
...
-- 	, `月度SKU采购频次`
	...
	, round((`在仓产品金额`+`上期在仓产品金额`)/2/usdratio/`发货订单采购金额usd`*7)  as `本地仓库存周转天数`
