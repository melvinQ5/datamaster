-- 新品链接，按账号，站点，人员，渠道。然后出一份按周度的，再一份按月度的。 0301-0515的新品链接，每个月的销量和广告情况
-- 	StartDay = 2023-05-01   NextStartDay= 今天
...
-- and ms.Site regexp 'UK|DE|FR|US'
join t_prod on wl.sku = t_prod.sku and ms.Site regexp 'UK|DE|FR|US|CA'
where wl.IsDeleted = 0 
	and ms.Department = '快百货' and wl.IsDeleted = 0 
	and MinPublicationDate  >= '${StartDay}'
...
left join t_elem on t_list.sku = t_elem.sku
)


-- select count(1) from t_merge
select * from t_merge 
order by NodePathName ,SellUserName ,shopcode ,sellersku

-- 各团队利润率
-- select nodepathname ,round(sum(利润额_扣广告)/sum(销售额),2) 利润率
-- from t_merge 
