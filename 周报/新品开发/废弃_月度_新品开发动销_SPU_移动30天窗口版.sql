...
where date_add(epp.DevelopLastAuditTime,interval - 8 hour) >=  date_add('${NextStartDay}',interval -30 - 14 day ) and date_add(epp.DevelopLastAuditTime,interval - 8 hour) < date_add('${NextStartDay}',interval - 14 day )
...
select * from (
	...
	from (
		...
		join import_data.mysql_store ms on ms.Code = od.shopcode and od.IsDeleted = 0
			and ms.Department ='快百货' and PayTime >= date_add('${NextStartDay}',interval -30 - 14 day )
		...
		left join ( select BoxSku, min(PayTime) as min_paytime from import_data.wt_orderdetails  od1
			...
			and ms1.Department ='快百货' and PayTime >= date_add('${NextStartDay}',interval -30 - 14 day )
...
select
    concat(date( date_add('${NextStartDay}',interval -30 - 14 day )) ,'至', date(date_add('${NextStartDay}',interval - 14-1 day )))  '终审时间范围'
     ,*
from (
	select
	    '日期' `分析维度` , DevelopUserName `开发人员`, dev_cnt `开发SPU数`
		...
		from (
		select    '开发合计' as DevelopUserName
	...
	union all
	select '日期/开发人员' `分析维度` , DevelopUserName `开发人员`, dev_cnt `开发sku数`, round(ord7_sku_cnt/dev_cnt,4) as d7_rate, round(ord14_sku_cnt/dev_cnt,4) as d14_rate, round(ord30_sku_cnt/dev_cnt,4) as d30_rate
...
-- 		where  DevelopUserName in('李云霞' ,'王婉君' ,'夏菲','左佩' ,'沈邦华','陈倩' ,'李琴1688' ,'丁华丽','金磊')
		group by t.DevelopUserName
		) tmp

	union all
	select '日期/开发团队'  `分析维度` , dep2 `开发团队`, dev_cnt `开发sku数`
