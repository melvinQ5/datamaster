...
, t1 as (  -- 维度1
	select '日期' `分析维度`, tmp.dev_month `终审月`, tmp.DevelopUserName `开发人员`, dev_cnt `开发SKU数`
		...
		select t.dev_month, '开发合计' as DevelopUserName
			, count(distinct t.sku) as dev_cnt
			...
			, count(distinct case when 0 <= ord_days_since_lst and ord_days_since_lst <= 7  then od.sku end) as ord7_sku_cnt_since_lst
			, count(distinct case when 0 <= ord_days_since_lst and ord_days_since_lst  <= 14 then od.sku end) as ord14_sku_cnt_since_lst
			, count(distinct case when 0 <= ord_days_since_lst and ord_days_since_lst  <= 30 then od.sku end) as ord30_sku_cnt_since_lst
        ...
        left join (
            select dev_month
                 ,count( case when ord14_orders_since_lst >= 1 then sku end ) as ord14_sale1_sku_cnt_since_lst
                 ,count( case when ord14_orders_since_lst >= 2 then sku end ) as ord14_sale2_sku_cnt_since_lst
                 ,count( case when ord30_orders_since_lst >= 1 then sku end ) as ord30_sale1_sku_cnt_since_lst
                 ,count( case when ord30_orders_since_lst >= 2 then sku end ) as ord30_sale2_sku_cnt_since_lst
                 ,count( case when ord30_orders_since_lst >= 3 then sku end ) as ord30_sale3_sku_cnt_since_lst
                 ,count( case when ord30_orders_since_lst >= 6 then sku end ) as ord30_sale6_sku_cnt_since_lst
                 ,count( case when ord30_orders_since_dev >= 3 then sku end ) as ord30_sale3_sku_cnt_since_dev
                 ,count( case when ord30_orders_since_dev >= 6 then sku end ) as ord30_sale6_sku_cnt_since_dev
            from
                ( select t.dev_month ,od.sku
                ...
                from tmp_epp t left join orders od on od.BoxSku =t.BoxSKU  group by t.dev_month ,od.sku
                ) ta
            group by dev_month
            ) tmp2 on tmp.dev_month =tmp2.dev_month
		left join ( select dev_month
            ,count( distinct  sku ) dev_pub_cnt
	...
	from ( select t.dev_month, t.DevelopUserName
	    ...
	    left join (
            select dev_month ,DevelopUserName
                 ...
                 ,count( case when ord30_orders_since_dev >= 6 then sku end ) as ord30_sale6_sku_cnt_since_dev
            from
                ( select t.dev_month ,t.DevelopUserName,od.sku
                    , count(distinct case when 0 <= ord_days_since_lst and ord_days_since_lst  <= 14 then PlatOrderNumber end) as ord14_orders_since_lst
                    , count(distinct case when 0 <= ord_days_since_lst and ord_days_since_lst  <= 30 then PlatOrderNumber end) as ord30_orders_since_lst
                    , count(distinct case when 0 <= ord_days and ord_days  <= 30 then PlatOrderNumber end) as ord30_orders_since_dev
                from tmp_epp t left join orders od on od.BoxSku =t.BoxSKU  group by t.dev_month ,t.DevelopUserName,od.sku
                ) ta
