...
,t_list as ( -- 盘活链接
select wl.shopcode ,wl.sellersku ,wl.sku ,spu ,nodepathname ,SellUserName ,m.c4 as activate_week ,MinPublicationDate ,ele
from manual_table m
left join import_data.wt_listing wl on m.handlename ='快百货盘活链接' and handletime = '2023-09-19' and wl.IsDeleted = 0 and m.c2 =wl.shopcode and m.c3=wl.sellersku
join import_data.mysql_store ms on wl.shopcode=ms.Code and department = '快百货'
left join t_elem on t_elem.sku = wl.sku
)
-- select * from t_list where ele is null

,t_list_stat as ( -- 盘活链接的渠道SKU统计
select ShopCode ,ele ,count( distinct concat( ShopCode , SellerSKU ) ) sellersku_cnt
from t_list m  group by ShopCode ,ele
)
...
-- select * from t_list_stat where ShopCode='A02-FR'

,t_orde_week_stat as ( -- 用于累计订单
select wo.shopcode ,ele ,dim_date.week_num_in_year as pay_week
...
group by wo.shopcode  ,ele  ,dim_date.week_num_in_year
)
...
from
	( select ta.shopcode  ,ta.ele ,week_num_in_year as ad_stat_week
		-- 曝光量
        ...
        group by ta.shopcode  ,ta.ele  ,ad_stat_week
	) tmp
)


,t_merage as (
select t1.shopcode ,t1.ele 元素
...
left join t_orde_week_stat t2 on t1.shopcode = t2.shopcode and t1.ele = t2.ele and t1.key_week = t2.pay_week and t2.pay_week <= t1.key_week +4
join mysql_store ms on t1.shopcode = ms.code
-- join t_list t2 on t1.shopcode = t2.shopcode and t1.sellersku = t2.sellersku
left join t_ad_stat t3 on t1.shopcode = t3.shopcode and t1.ele = t3.ele and t1.key_week = t3.ad_stat_week and t3.ad_stat_week <= t1.key_week +4
left join t_list_activate_week_stat t5 on t1.shopcode = t5.shopcode
)


select * from t_merage
order by ShopCode ,元素 ,订单周