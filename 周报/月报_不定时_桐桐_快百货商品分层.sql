...
,ele as (
select p.BoxSku ,GROUP_CONCAT( Name ) as ele_name 
from import_data.erp_product_product_associated_element_attributes eppaea
left join import_data.erp_product_product_element_attributes eppea on eppaea.ElementAttributeId = eppea.Id
join wt_products p on eppaea.sku = p.sku and p.ProjectTeam='快百货'
group by p.BoxSku  
)
...
, t_merge as (
 select 
		...
		,wp.ProductName `产品名` 
	 	...
	 	,wp.CategoryPathByChineseName `类目节点`
	 	,wp.Festival `季节`
	 	,ele.ele_name  `元素`
	 	,wp.SPU 
	from 
	...
	left join ele on list.BoxSku  = ele.BoxSku
