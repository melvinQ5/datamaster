...
select distinct epp.spu ,epp.sku ,PushDate ,PushSite 主推站点,PushRule 推送标准,PushUser 推送人 ,PushReason 推送理由
    ,case when '${NextStartDay}' > StopPushDate then '是' else '否' end 是否过季
from erp_product_products epp
join dep_kbh_product_level_potentail t on  epp.spu =t.spu
where PushDate >= '2023-08-01' and epp.IsMatrix=0 and epp.IsDeleted=0 and ProjectTeam='快百货' and isStopPush ='否'
)
...
from t_list
where lst_days > 0
group by sku,PushDate
)
-- select * from t_list_stat

,t_orde as (  -- 主键 spu x 推送日期
select
   wo.Product_SKU as SKU ,PushDate
...
join t0 on wo.Product_Sku  = t0.sku -- 一变多
where
	PayTime >= date_add('2023-10-01', interval -30 day)  -- 0703是首批推送日期，有一个指标需要计算推荐前30天，故使用0703往前推30天作为固定起始时间
...
,t_ad as (
select t0.sku, waad.GenerateDate, waad.ShopCode ,waad.Asin , waad.AdClicks, waad.AdExposure, waad.AdSaleUnits
	, PushDate
	, timestampdiff(SECOND,PushDate,waad.GenerateDate)/86400 as ad_days -- 广告
from wt_adserving_amazon_daily waad
join t0 on waad.sku = t0.sku
...
from
	( select  sku,PushDate
		...
		from t_ad  group by  sku,PushDate
	) tmp
)
...
,t_merage as (
select t0.sku ,t0.PushDate 推送日期
	,wp.ProductName
...
left join (select distinct sku ,ele_name_group ,ele_name_priority,productstatus  from dep_kbh_product_test ) dt on t0.sku=dt.sku
left join wt_products wp on t0.SKU=wp.SKU
left join t_list_stat on t0.sku =t_list_stat.sku and t0.pushdate =t_list_stat.pushdate
left join t_ad_stat on t0.sku =t_ad_stat.sku and t0.pushdate =t_ad_stat.pushdate
left join t_orde_stat on t0.sku =t_orde_stat.sku and t0.pushdate =t_orde_stat.pushdate
left join online_companycode oc on oc.sku = t0.sku and t0.pushdate =oc.pushdate
