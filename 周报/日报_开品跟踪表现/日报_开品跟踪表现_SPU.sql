...
from (select eppaea.SPU  ,eppea.Name
...
from t_orde
join (
	select spu ,min(MinPublicationDate) as min_pub_date from t_list group by spu
	) tmp 
...
left join 
	...
	from erp_product_products wp where IsMatrix = 1  and CreationTime  >= '${StartDay}' and ProjectTeam='快百货' 
	) ta on epp.SPU =ta.spu
left join (
	select SPU 
		,GROUP_CONCAT(TortType)   TortType 
		,GROUP_CONCAT(Artist) Artist 
		,GROUP_CONCAT(Editor) Editor 
		,GROUP_CONCAT(Festival) Festival 
	from (
		select SPU ,TortType ,Artist ,Editor ,Festival
		from wt_products wp2 
		where CreationTime  >= '${StartDay}' and ProjectTeam='快百货' 
		group by SPU ,TortType ,Artist ,Editor ,Festival
		) tb 
	group by spu 
...
left join online_companycode oc on oc.SPU = epp.SPU
