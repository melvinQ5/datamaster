...
,t_BadDebtRate as (
select '快百货' as dep  ,sum(FrozenAmountUs) `坏账金额` -- 使用实扣冻结额（美元） 作为计算坏账的金额字段
from import_data.BadDebtRate 
where to_date(ExceptionNotifyTime) >= '${StartDay}' and to_date(ExceptionNotifyTime) < '${NextStartDay}' 
...
left join t_BadDebtRate on t_key.dep = t_BadDebtRate.dep
order by `团队` desc
