...
,today_order_rate as ( -- 采购当天下单
select '快百货' as dep 
	,round(count(distinct case when timestampdiff(second,GenerateTime,OrderTime)<86400 then OrderNumber end)/count(distinct OrderNumber),4)  采购当天下单率
from daily_PurchaseOrder pc
join (SELECT d.Name 'team'
     ,u.Name salesman
     FROM erp_user_abpusers u
     inner join erp_user_user_departments  d
     on  u.departmentId = d.id
     where d.Name = '采购组' -- 快百货
     ) s
on pc.OrderPerson=s.salesman
where OrderTime>=date_add('${StartDay}',interval -2 day)
and OrderTime<date_add('${NextStartDay}',interval -2 day)
)
...
left join today_order_rate on t_key.dep = today_order_rate.dep
)
