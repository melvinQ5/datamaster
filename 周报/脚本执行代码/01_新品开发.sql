...
-- step1 数据源处理
t_prod as (
select SKU ,ProjectTeam ,DevelopLastAuditTime ,IsMatrix ,spu ,CreationTime ,boxsku ,SkuSource ,Status ,ProductStatus 
...
select CASE WHEN ProjectTeam  IS NULL THEN '公司' ELSE ProjectTeam  END AS dep
	...
	,count(distinct t_prod.sku ) `新品SkU数` 
...
where IsMatrix = 0 and date_add(DevelopLastAuditTime,interval - 8 hour) < '${NextStartDay}' 
	and date_add(DevelopLastAuditTime,interval - 8 hour) >= '${StartDay}' 
...
,t_new_add_spu as (
select CASE WHEN ProjectTeam  IS NULL THEN '公司' ELSE ProjectTeam  END AS dep
	,count(distinct t_prod.spu ) `添加新品SPU数` 
from t_prod
left join t_copy_new_pp on t_prod.spu = t_copy_new_pp.spu
where IsMatrix = 1 and CreationTime < '${NextStartDay}' and CreationTime >= '${StartDay}' 
	and Status != 20 and t_copy_new_pp.spu is null 
group by grouping sets ((),(ProjectTeam))
)
...
select 
	...
	,t_new_spu.`新品SPU数` 
	,t_new_spu.`新品SKU数` 
	,round(`新品SKU数`/`新品SPU数`,2) as `新品变体配比`
	,t_new_spu.`元素新品SPU数`
	,t_new_add_spu.`添加新品SPU数`
...
left join t_new_add_spu on t_key.dep = t_new_add_spu.dep 
order by `团队` desc 
