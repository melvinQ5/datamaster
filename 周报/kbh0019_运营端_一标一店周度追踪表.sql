...
select lm.ShopCode
     -- , round(TotalCount*FeaturedOfferPercent/100,0) `访客数`
     , TotalCount `访客总数`
     ,OrderedCount `访客销量` ,week_num_in_year
...
select ShopCode ,week_num_in_year as lm_week
    ,sum( 访客总数 ) 访客总数
    ,round ( sum( 访客销量 ) / sum( 访客总数 ) ,4 ) 访客转化率
from vist
...
where Department = '快百货'
  and CompanyCode regexp 'YV|A02|ZR|XS|YH|YK|ZH|YT|YY|YU|YP' -- 一标一店范围
)
...
,访客总数
,访客转化率
,round(ifnull(访客总数,0) - ifnull(ad_sku_Clicks,0)) `自然访客数（减法）`
